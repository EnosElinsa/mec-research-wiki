...
$$
\begin{array}{l} \phi_ {\mathrm{approx}} = \phi + \frac {\alpha}{Z _ {1}} \mathbb {E} _ {s \sim p _ {\pi}} \left[ \nabla_ {\phi} \pi_ {\phi} (s) \nabla_ {a} Q _ {\theta} (s, a) | _ {a = \pi_ {\phi} (s)} \right] \\ \phi_ {\text { true }} = \phi + \frac {\alpha}{Z _ {2}} \mathbb {E} _ {s \sim p _ {\pi}} \left[ \nabla_ {\phi} \pi_ {\phi} (s) \nabla_ {a} Q ^ {\pi} (s, a) | _ {a = \pi_ {\phi} (s)} \right], \tag {4} \\ \end{array}
$$

where we assume $Z _ { 1 }$ and $Z _ { 2 }$ are chosen to normalize the gradient, i.e., such that $Z ^ { - 1 } | | \mathbb { E } [ \cdot ] | | = 1$ . Without normalized gradients, overestimation bias is still guaranteed to occur with slightly stricter conditions. We examine this case further in the supplementary material. We denote πapprox and $\pi _ { \mathrm { t r u e } }$ as the policy with parameters $\phi _ { \mathrm { a p p r o x } }$ and $\phi _ { \mathrm { t r u e } }$ respectively.

![](images/0285e3c0f80495e50ea7937e08c6c77e4b0371193109c8f4c0ca8177678f3704.jpg)

<details>
<summary>line</summary>

| Time steps (1e6) | CDQ   | True CDQ | DDPG  | True DDPG |
| ---------------- | ----- | -------- | ----- | --------- |
| 0.0              | 0     | 0        | 0     | 0         |
| 0.2              | 200   | 200      | 200   | 200       |
| 0.4              | 280   | 280      | 280   | 280       |
...
![](images/95c10a198b7dd399cecdf9bd1e5b2b3b0078ef76fec7dad4fded74755f8bddb9.jpg)

<details>
<summary>line</summary>

| Time steps (1e6) | Blue Line Value | Orange Line Value |
| ---------------- | --------------- | ----------------- |
| 0.0              | 0               | 0                 |
| 0.2              | 150             | 200               |
| 0.4              | 250             | 300               |
...
![](images/2bf70c7dd31cfbe773283d6bbae2845222a5df1566378d84ff31f9aae92fc593.jpg)  
...
$$
y _ {1} = r + \gamma Q _ {\theta_ {2} ^ {\prime}} (s ^ {\prime}, \pi_ {\phi_ {1}} (s ^ {\prime})) \tag {9}
$$

$$
y _ {2} = r + \gamma Q _ {\theta_ {1} ^ {\prime}} (s ^ {\prime}, \pi_ {\phi_ {2}} (s ^ {\prime})).
$$
...
![](images/48be1180738538cfc0d0178cc0d58bdd99f6e073b2e17be19aa12c6657f82ded.jpg)

<details>
<summary>line</summary>

| Time steps (1e5) | Average Value (τ = 1) | Average Value (τ = 0.01) | Average Value (τ = 0.1) | True Value |
| ---------------- | --------------------- | ------------------------ | ----------------------- | ---------- |
| 0.0              | ~330                  | ~150                     | ~330                    | ~330       |
| 0.2              | ~325                  | ~280                     | ~325                    | ~325       |
| 0.4              | ~325                  | ~320                     | ~325                    | ~325       |
...
![](images/b3607dc35cc207a85976fd1dacee8ff34c959f8b13cd81c5997eeb175d985680.jpg)

<details>
<summary>line</summary>

| Time steps (1e5) | Series 1 | Series 2 | Series 3 |
| ---------------- | -------- | -------- | -------- |
| 0.0              | 10000    | 1000     | 100      |
| 0.2              | 1000     | 1000     | 100      |
| 0.4              | 1000     | 1000     | 100      |
...
While updating the value estimate without target networks (τ = 1) increases the volatility, all update rates result in similar convergent behaviors when considering a fixed policy. However, when the policy is trained with the current value estimate, the use of fast-updating target networks results in highly divergent behavior.
...
$$
y = r + \gamma Q _ {\theta^ {\prime}} (s ^ {\prime}, \pi_ {\phi^ {\prime}} (s ^ {\prime}) + \epsilon), \tag {14}
$$
...
Select action with exploration noise $a \sim \pi(s) + \epsilon$ , $\epsilon \sim \mathcal{N}(0, \sigma)$ and observe reward $r$ and new state $s'$ Store transition tuple $(s, a, r, s')$ in $\mathcal{B}$ Sample mini-batch of $N$ transitions $(s, a, r, s')$ from $\mathcal{B}$ $\tilde{a} \leftarrow \pi_{\phi'}(s) + \epsilon, \quad \epsilon \sim \text{clip}(\mathcal{N}(0, \tilde{\sigma}), -c, c)$ $y \leftarrow r + \gamma \min_{i=1,2} Q_{\theta'_i}(s', \tilde{a})$ Update critics $\theta_i \leftarrow \min_{\theta_i} N^{-1} \sum (y - Q_{\theta_i}(s, a))^2$ if $t \mod d$ then
...
![](images/a378336577c8cd2625d02610afacd36708870a19623296d05b8766613146598f.jpg)  
...
$$
y = r + \gamma \min _ {i = 1, 2} Q _ {\theta_ {i} ^ {\prime}} (s ^ {\prime}, \pi_ {\phi^ {\prime}} (s ^ {\prime}) + \epsilon), \tag {15}
$$
...
![](images/d71c0679b80e322cf251450f514ba0de8a55aea4e5871e54aee05e9e26f2cc9b.jpg)

Figure 5. Learning curves for the OpenAI gym continuous control tasks. The shaded region represents half a standard deviation of the average evaluation over 10 trials. Some graphs are cropped to display the interesting regions.   
...
<table><tr><td>Environment</td><td>TD3</td><td>DDPG</td><td>Our DDPG</td><td>PPO</td><td>TRPO</td><td>ACKTR</td><td>SAC</td></tr><tr><td>HalfCheetah</td><td> $9636.95 \pm 859.065$ </td><td>3305.60</td><td>8577.29</td><td>1795.43</td><td>-15.57</td><td>1450.46</td><td>2347.19</td></tr><tr><td>Hopper</td><td> $3564.07 \pm 114.74$ </td><td>2020.46</td><td>1860.02</td><td>2164.70</td><td>2471.30</td><td>2428.39</td><td>2996.66</td></tr><tr><td>Walker2d</td><td> $4682.82 \pm 539.64$ </td><td>1843.85</td><td>3098.11</td><td>3317.69</td><td>2321.47</td><td>1216.70</td><td>1283.67</td></tr><tr><td>Ant</td><td> $4372.44 \pm 1000.33$ </td><td>1005.30</td><td>888.77</td><td>1083.20</td><td>-75.85</td><td>1821.94</td><td>655.35</td></tr><tr><td>Reacher</td><td> $-3.60 \pm 0.56$ </td><td>-6.51</td><td>-4.01</td><td>-6.18</td><td>-111.43</td><td>-4.26</td><td>-4.44</td></tr><tr><td>InvPendulum</td><td> $1000.00 \pm 0.00$ </td><td>1000.00</td><td>1000.00</td><td>1000.00</td><td>985.40</td><td>1000.00</td><td>1000.00</td></tr><tr><td>InvDoublePendulum</td><td> $9337.47 \pm 14.96$ </td><td>9355.52</td><td>8369.95</td><td>8977.94</td><td>205.85</td><td>9081.92</td><td>8487.15</td></tr></table>
...
The target policy smoothing is implemented by adding  ∼ $\mathcal { N } ( 0 , 0 . 2 )$ to the actions chosen by the target actor network, clipped to (−0.5, 0.5), delayed policy updates consists of only updating the actor and target critic network every d iterations, with d = 2. While a larger d would result in a larger benefit with respect to accumulating errors, for fair comparison, the critics are only trained once per time step, and training the actor for too few iterations would cripple learning. Both target networks are updated with $\tau = 0 . 0 0 5$ .
