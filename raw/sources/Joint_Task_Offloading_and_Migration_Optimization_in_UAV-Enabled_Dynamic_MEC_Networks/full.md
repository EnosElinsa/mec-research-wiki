...
HE rise of wireless technologies and mobile devices has fueled the growth of computationally intensive, latencysensitive applications like real-time video analysis and online

Received 8 January 2025; revised 26 May 2025; accepted 27 May 2025. Date of publication 4 June 2025; date of current version 8 August 2025. This work was supported in part by the National Natural Science Foundation of China under Grant 62332014, and in part by the Natural Science Foundation of Shaanxi Province for Distinguished Young Scholars under Grant 2023-JC-JQ-54. (Corresponding author: Liang Wang.)

Liang Wang, Bingnan Shen, Yao Zhang, and Bin Guo are with the School of Computer Science, Northwestern Polytechnical University, Xi’an 710129, China (e-mail: liangwang0123@gmail.com).

Lianbo Ma is with the College of Software, Northeastern University, Shenyang 110819, China.

Yingnan Zhao is with the College Of Computer Science And Technology, Harbin Engineering University, Harbin 150001, China.

Hongzhi Guo is with the School of Cybersecurity, Northwestern Polytechnical University, Xi’an 710129, China.

Zhiwen Yu is with the School of Computer Science and Technology, Harbin Engineering University, Harbin 150001, China, and also with the School of Computer Science, Northwestern Polytechnical University, Xi’an 710129, China.

Digital Object Identifier 10.1109/TSC.2025.3576644

gaming, which require high Quality of Service (QoS) [1]. However, mobile devices struggle with processing these tasks due to limited battery life and computational power. In recent years, Multi-access Edge Computing (MEC) [2] has emerged as an effective solution by bringing computational resources closer to Mobile Users (MUs) through edge servers at the network’s edge, such as base stations and access points. Offloading tasks to these edge servers can enhance QoS for MUs in a cost-effective and energy-efficient manner.
...
![](images/706d2885436ac2b80142d73b64fa3310a6d47deea1bc4a3266367e0c1028bab2.jpg)
...
```mermaid
graph TD
    subgraph_Timeframe["Timeframe Diagram"]
        A["Fast Link"] --> B["Hard Delay Task"]
        C["Slow Link"] --> D["Soft Delay Task"]
        E["Distance(meters)"] --> F["SDN Controller"]
        G["Speed"] --> H["UAV 1..."]
        I["Speed"] --> J["UAV 2..."]
        K["Speed"] --> L["UAV 3..."]
        M["Speed"] --> N["UAV 4..."]
    ...
    subgraph_Timeframe["Timeframe Diagram"]
        Q["Migration path"] --> R["Offload/Download"]
        S["MUs’Moving Trajectory"] --> T["Available Resource"]
    end

    subgraph_Timeframe["Timeframe Diagram"]
        U["SDN Controller"] --> V["t1 MU 1..."]
        W["t2 MU 1..."]
        X["t1 MU 2..."]
        Y["t2 MU 2..."]
        Z["t1 MU 3..."]
    ...
    subgraph_Timeframe["Timeframe Diagram"]
        AE["Expected Return Time of Task Result"] --> AF["Channel Condition"]
        AG["UAV Computational Capabilities"] --> AH["Task Type and Task Deadline"]
        AI["Decision Making"] --> AJ["Task Offloading"]
        AK["Task Migration"] --> AL["State Update"]
        AM["MD Mobility"] --> AN["Network Fluctuation"]
        AO["User Arrival"] --> AP["Resource Fluctuation"]
    end

    subgraph_Timeframe["Timeframe Diagram"]
        AQ["Rate(Mbps)"] --> AR["A2G"]
        AS["Distance(meters)"] --> AT
    end
```
</details>
...
```mermaid
graph TD
    A["Geographical Locations"] --> B["Execution UAV 1"]
    B --> C["Offloading"]
    C --> D["MU 1"]
    D --> E["Download"]
    E --> F["UAV 2"]
    F --> G["Operation Status"]
    F --> H["Channel Conditions"]
    H --> I["SDN Controller"]
    I --> J["Operation Status"]
    I --> K["Channel Conditions"]
    K --> L["UAV 3"]
    L --> M["Operation Status"]
    L --> N["Download"]
    N --> O["UAV 4"]
    O --> P["Operation Status"]
    O --> Q["Migration"]
    Q --> R["UAV 5"]
    R --> S["Operation Status"]
    R --> T["Downlink"]
    T --> U["UAV Selection for Execution"]
    U --> V["UAV Association"]
    
    W["Dynamic Task Offloading and Migration Workflow"] --> X["UAV Switching"]
    X --> Y["Remain Unchanged"]
    Y --> Z["Yes"]
    Y --> AA["No"]
    Z --> AB["1 + 2 + 3 + 4 > Deadline?"]
    AA --> AC["Migration Trigger Condition"]
    AC --> AD["UAV Migration Decision"]
    AD --> AE["↑"]
    AE --> AF["UAV Selection for Execution"]
    AF --> AG["UAV Association"]
```
</details>

Fig. 2. The system architecture and workflow.

During the time horizon T , UAVs remain stationary at predefined positions with a fixed altitude $h ,$ where each UAV $j \in \mathcal { U }$ is located at $l o c _ { i } ^ { u a v }$ . The UAVs’ key parameters, including CPU clock speed $\varpi _ { j }$ (cycles per second,  GHz $1 0 ^ { 9 }$ cycles per second), computation intensity $\rho _ { j }$ 1 =(CPU cycles 10per bit), and wireless coverage radius $r a d _ { j }$ , define their computational capacity and communication range. In contrast, MUs are continuously moving through the system but are assumed to remain static within each time slot. The position of MU i at time slot t is denoted by $l o c _ { i , t } ^ { m u }$ . Each MU i is assumed to have a computationally intensive task $\Gamma _ { i } ,$ such as processing Γhigh-definition 3D road maps, which can be offloaded to a UAV for execution. Task generation times of all MUs are randomly distributed over the time horizon $\tau$ , with each MU’s task arriving independently. To capture the stochastic nature of task generation, task arrivals are modeled as a Poisson process, i.e., a widely adopted assumption in communication systems. Tasks are categorized based on their latency constraints: hard delay tasks, which must be processed and returned within a strict deadline $d l _ { i } .$ , and soft delay tasks, which can tolerate some latency at the expense of degraded QoS as delay increases [38]. Each task $\Gamma _ { i }$ is characterized by the tuple $\Gamma _ { i } \doteq \langle d s _ { i } , \mu _ { i } , r e s _ { i } , \xi _ { i } \rangle$ , where $d \boldsymbol { s } _ { i }$ is the input data size in bits, $\mu _ { i }$ =is is the computational demand in CPU cycles per second (Hz), resi represents the output data size, and $\xi _ { i }$ indicates whether the task has a hard or soft delay requirement. $\operatorname { I f } \xi _ { i } = 1$ , task $\Gamma _ { i }$ is classified as having a hard delay = 1 Γrequirement; otherwise, it is considered a soft delay task. The maximum delivery deadline $\chi _ { i }$ for returning the computation result of task $\Gamma _ { i }$ is defined as follows:

$$
\chi_ {i} = \left\{ \begin{array}{l l} d l _ {i}, & \text { if } \xi_ {i} = 1; \\ d l _ {i} + \Delta t _ {i}, & \text { if } \xi_ {i} = 0; \end{array} \right. \tag {1}
$$

where $d l _ { i }$ represents the deadline for task $\Gamma _ { i }$ and $\Delta t _ { i }$ denotes the Γ Δmaximum allowable delay beyond this deadline for soft delay tasks. Within this extended time, the task remains valid, though its performance may be degraded.

Task Offloading and Migration: To offload the computation task $\Gamma _ { i } ,$ MU i must upload the task to the MEC system via Γan accessible UAV $j \in \mathcal { U }$ . At each time slot t, MU i may be within the coverage area of multiple UAVs, represented by the set ${ \mathcal { U } } _ { i } ^ { t }$ . Among these UAVs, we adopt the widely used proximity-based UAV association scheme, which assigns the MU to the nearest UAV [39]. Let $\boldsymbol { x } _ { i , j } ^ { t }$ represent the association indicator between UAV $j \in \mathcal { U } _ { i } ^ { t }$ and MU $i \in \mathcal { M }$ at time slot t, where $x _ { i , j } ^ { t } = 1$ indicates that MU i is connected to UAV $j ;$ and $x _ { i , j } ^ { t } = 0$ 1otherwise. Formally, the association indicator is = 0determined directly as follows:

$$
x _ {i, j} ^ {t} = \underset {j \in \mathcal {U} _ {i} ^ {t}} {\arg \min} \left\| l o c _ {i, t} ^ {m u}, l o c _ {j} ^ {u a v} \right\|, \tag {2}
$$

where $\begin{array} { r } { \sum _ { j \in \mathcal { U } _ { \ast } ^ { t } } { x _ { i , j } ^ { t } } = 1 , \forall i \in \mathcal { M } , t \in \mathcal { T } } \end{array}$ , ensuring that each MU iconnects to only one UAV at any given time.
...
$$
\begin{array}{l} \mathrm{PL} _ {\mathrm{A2G}} (d _ {a g}, h) = 2 0 \log \left(\frac {4 \pi f _ {c} \sqrt {h ^ {2} + d _ {a g} ^ {2}}}{c}\right) \\ + P _ {\mathrm{LoS}} (d _ {a g}, h) \eta_ {\mathrm{LoS}} + (1 - P _ {\mathrm{LoS}} (d _ {a g}, h)) \eta_ {\mathrm{NLoS}}, \tag {4} \\ \end{array}
$$
...
$$
\mathrm{PL} _ {\mathrm{A2A}} (d) = 3 2. 4 5 + 2 0 \log f _ {c} + 2 0 \log d, \tag {6}
$$
...
$$
T R _ {i, j} ^ {t} = B (t) \log_ {2} \left(1 + \frac {E _ {\mathrm{i}} 1 0 ^ {- \frac {\mathrm{PL}}{1 0}}}{B (t) \sigma^ {2}}\right), \tag {7}
$$
...
$$
\hat {\mathcal {L}} _ {i, k, t} ^ {u p} = x _ {i, j} ^ {t} \sum_ {k \in \mathcal {U}} y _ {i, k} ^ {t} \sum_ {\{s, s + 1 \} \in P a t h _ {i, j, k} ^ {t}} \frac {d s _ {i}}{T R _ {s , s + 1} ^ {t}}, \tag {8}
$$

where xti,j $x _ { i , j } ^ { t } = 1 , y _ { i , k } ^ { t }$ represent the potential UAV serving MU i at time slot $t ,$ 1 and $T R _ { s , s + 1 } ^ { t }$ denotes the data transmission rate between consdelivery path $P a t h _ { i , j , k } ^ { t }$ ansmission nodes  and during the same time sl $s + 1$ along the

(2) Task Execution Latency: For newly arriving tasks, task execution latency is calculated based on the potential serving UAVs and the full input data. In contrast, for ongoing tasks, the latency calculation must account for both the time already spent processing the partial input data and the estimated time required to process the remaining data. At time slot t, for a newly offloaded task $\Gamma _ { i }$ offloaded from MU $i ,$ the involved execution Γlatency with respect to the potential serving UAV k is calculated as below:

$$
\hat {\mathcal {L}} _ {i, k, t} ^ {c o m p} = \sum_ {k \in \mathcal {U}} y _ {i, k} ^ {t} * \frac {d s _ {i} * \rho_ {k}}{\mu_ {i}}, \tag {9}
$$

where $y _ { i , k } ^ { t }$ denotes the potential serving UAV for task $\Gamma _ { i }$ at time $t , d s _ { i }$ is the task’s data size, $\mu _ { i }$ Γrepresents the computation demand, and $\rho _ { k }$ is UAV k’s computational power.

If $\Gamma _ { i }$ is an ongoing task, the SDN controller periodically Γcalculates its execution latency at each time slot. At time slot $t ,$ assuming the task remains on the current serving UAV k without migrating, the estimated execution latency $\hat { \mathcal { L } } _ { i , k , t } ^ { c o \overline { { { m } } } p }$ is given by:

$$
\hat {\mathcal {L}} _ {i, k, t} ^ {c o m p} = \sum_ {k \in \mathcal {U}} y _ {i, k} ^ {t} * \frac {d s _ {i} ^ {t} * \rho_ {k}}{\mu_ {i}} + l a _ {c o m p}, \tag {10}
$$

where $\mathit { d s } _ { i } ^ { t }$ represents the remaining data to be processed for task $\Gamma _ { i }$ , and $l a _ { c o m p }$ denotes the execution time already spent before Γthe current time slot t, calculated as follows:

$$
l a _ {c o m p} = \sum_ {n = 1} ^ {t} \sum_ {o \in \mathcal {U}} y _ {i, o} ^ {n - 1} * \frac {\left(d s _ {i} ^ {n - 1} - d s _ {i} ^ {n}\right) * \rho_ {o}}{\mu_ {i}}, \tag {11}
$$

where yn−i,o $y _ { i , o } ^ { n - 1 }$ represents the UAV serving task i during time slot $n - 1$ .

1(3) Task Migration Latency: At time slot t, if task $\Gamma _ { i }$ requires Γmigration to another UAV to satisfy QoS requirements, the remaining input data $d s _ { i } ^ { t }$ (for continued execution) and the partial result obtained so far must be transferred to the newly assigned UAV. For simplicity, the partial result for task $\Gamma _ { i }$ at time t is represented as $\zeta _ { t } \times r e s _ { i } ,$ where $0 < \zeta _ { t } < 1$ Γis a proportionality 0 1coefficient representing the fraction of the final output result. This process assumes that migration is completed within a single time slot. Since multiple migrations may occur during task execution, the migration latency is calculated recursively. Specifically, if task $\Gamma _ { i } ,$ currently served by UAV k, is migrated Γto a potential serving UAV o via the selected transmission path $P a t h _ { k  o } ^ { t }$ →o at time slot t (where $k \neq o )$ , the migration latency ˆi,k,o,t is given by $\hat { \mathcal { L } } _ { i , k , o , t } ^ { m i g }$ Lmig

$$
\hat {\mathcal {L}} _ {i, k, o, t} ^ {m i g} = \sum_ {\{s, s + 1 \} \in P a t h _ {k \rightarrow o} ^ {t}} \frac {d s _ {i} ^ {t} + \zeta_ {t} \times r e s _ {i}}{T R _ {s , s + 1} ^ {t}} + \mathcal {L} _ {i, t - 1} ^ {m i g} \tag {12}
$$

where $\{ s , s + 1 \}$ represents consecutive nodes along the trans-+mission path $P \dot { a } t h _ { k  o } ^ { \bar { t } }$ from UAV k to UAV o. Additionally, $\mathcal { L } _ { i , t - 1 } ^ { m i g }$ 1 represents the cumulative migration latency experienced by MU i up to the current time slot t, and this value is logged in the system’s operational records.
...
$$
\hat {\mathcal {L}} _ {k, i, t} ^ {\text { down }} = x _ {i, j} ^ {t} \sum_ {k \in \mathcal {U}} y _ {i, k} ^ {t} \sum_ {\{s, s + 1 \} \in \operatorname{Path} _ {k, j, i} ^ {t}} \frac {\operatorname{res} _ {i}}{T R _ {s , s + 1} ^ {t}}, \tag {13}
$$

where $P a t h _ { k , j , i } ^ { t }$ presents the delivery path from UAV k, whichi $\Gamma _ { i } ,$ Γconnected UAV j at time slot t. The transmission rate between $T R _ { s , s + 1 } ^ { t }$ ive no, while $r e s _ { i }$ and refer $s + 1$ along this path is denoted bye size of the computation result.
...
![](images/316d557267fd6ff47c326f5d28af33ebd86eb008a80e628ad03cdbf98cc9848a.jpg)
...
```mermaid
graph TD
    A["Expert policy"] --> B["IPPO training"]
    A --> C["Trajectories"]
    B --> D["Environment"]
    C --> D
    D --> E["agent policy"]
    ...
    I --> K["Compute loss"]
    I --> L["<State-actions>"]
    E --> M["Offline pretrain"]
    M --> N["Expert dataset request"]
    N --> D
    D --> O["state action reward"]
    D --> P["state action reward"]
    D --> Q["old state Execute action"]
    Q --> D
    D --> R["new state"]
    R --> D
    R --> S["Expert dataset"]
...
$$
\begin{array}{l} J _ {p p o} (\theta_ {e}) = \mathbb {E} _ {t} \left[ \sum_ {i = 0} ^ {M} \min \left(\varrho_ {t} ^ {i} (\theta_ {e}) \hat {A} _ {t} ^ {i}, \right. \right. \\ \left. \left. \operatorname{clip} (\varrho_ {t} ^ {i} (\theta_ {e}), 1 - \varepsilon , 1 + \varepsilon) \hat {A} _ {t} ^ {i}\right) \right], \tag {19} \\ \end{array}
$$

$$
\hat {A} _ {t} ^ {i} = \sum_ {l = 0} ^ {T - t - 1} (\gamma \lambda) ^ {l} \left(r _ {t + l} ^ {i} + \gamma V (s _ {t + l + 1} ^ {i}) - V (s _ {t + l} ^ {i})\right), \tag {20}
$$
...
$$
\mathbb {L} (\psi_ {e}) = \mathbb {E} _ {t} [ (V _ {\psi_ {e}} (s _ {t} ^ {i}) - V _ {t a r g e t}) ^ {2} ], \tag {21}
$$
...
![](images/818d90514af5d982dd8f8259379110fd29491540b02d8c5087bdbe9ed381d8f8.jpg)
...
```mermaid
graph TD
    A["Environment"] --> B["Critic Network"]
    A --> C["Actor Network"]
    A --> D["Reward function"]
    B --> E["Select action"]
    C --> F["Feature Fusion"]
...
Input: policy params $\theta_{e}$ , value params $\psi_{e}$ , replay buffer G;
Output: optimal $\theta_{e}^{*}$ ;
1: for i_episode = 0, 1, ... do
2: Initialize environment, t = 0, obtain the state $s_{0}$ ;
3: while not done do
4: for i in range(M) do
5: Obtain the state $s_{t}^{i}$ ;
6: Choose an action $a_{t}^{i}$ based on policy $\theta_{e}^{old}$ ;
7: if G is full then
8: Compute advantage estimates $\hat{A}_{t}^{i}$ based on
9: the value function $\mathbb{L}(\psi_{e})$ ;
10: for k = 0 to K do
11: Optimize $J_{\mathrm{PPO}}(\theta_{e})$ ;
12: end for
13: $\theta_{e}^{old} \leftarrow \theta_{e}$ ;
14: $\psi_{e}^{old} \leftarrow \psi_{e}$ ;
15: Update $\eta$ based $\theta_{e}^{old}$ and $\psi_{e}^{old}$ ;
16: end if
17: end for
18: $t = t + 1$ ;
19: end while
20: if $-\frac{1}{M}\sum_{i=0}^{M}L_{i}$ for this episode <= $\eta$ then
21: Collect the episode's trajectories into G;
22: end if
23: end for
...
Step 1. Neural Network Initialization: At the outset, the agent network parameters are initialized. The agent refines its policy by minimizing the discrepancy between its own stateaction pairs and those of the expert. To facilitate this, two networks are employed: the generator and the discriminator. The discriminator network $D _ { \phi _ { a } }$ , distinguishes between expert and aagent-generated behaviors. The generator comprises two key components: the policy network, which optimizes the agent’s decision-making, and the value network, which evaluates the quality of the agent’s selected strategies. The agent selects actions $a _ { t } ^ { i , a g e n t }$ based on the observed state $s _ { t } ^ { i } .$ refining its policy by aligning the distribution of its stateaction pairs, $\pi ^ { \dot { \theta _ { a } } } ( a _ { t } ^ { \dot { i } , a g e n \bar { t } } | s _ { t } ^ { i } )$ , with the expert’s distribution, $\pi ^ { \theta _ { e } } ( a _ { t } ^ { i , e x p e r t } | s _ { t } ^ { i } )$ ( ). The discriminator network is employed to dis-the state-action pairs generated by the expert and those produced by the agent. Additionally, the value network assesses the quality of the agent’s actions by comparing how closely they resemble the expert’s strategy versus its own learned policy. The loss function for this discriminator network is defined as below:

$$
\begin{array}{l} \mathbb {L} (\phi_ {a}) = - \mathbb {E} _ {\rho_ {\pi^ {\theta_ {e}}}} [ \log D _ {\phi_ {a}} (s _ {t} ^ {i}, a _ {t} ^ {i, e x p e r t}) ] \\ \left. - \mathbb {E} _ {\rho_ {\pi^ {\theta_ {a}}}} \left[ \log \left(1 - D _ {\phi_ {a}} \left(s _ {t} ^ {i}, a _ {t} ^ {i, a g e n t}\right)\right) \right], \right. \tag {23} \\ \end{array}
$$

where $\rho _ { \pi ^ { \theta _ { e } } } [ \cdot ]$ denotes the expected value of state-action traθe [ ]jectories generated by the expert policy $\pi ^ { \theta _ { e } }$ , whereas $\rho _ { \pi ^ { \theta _ { a } } } [ \cdot ]$ a [ ]represents the expected value of state-action trajectories generated by the agent’s policy $\pi ^ { \theta _ { a } }$ . Specifically, the term $- \mathbb { E } _ { \rho _ { \pi ^ { \theta _ { e } } } } [ \log D _ { \phi _ { a } } ( s _ { t } ^ { i } , a _ { t } ^ { i , e x p e r t } ) ) ]$ is designed to maximize the action pairs generated by the expert policy as originating from the expert. This incentivizes the generator to produce behaviors that closely align with those of the expert. The term $- \mathbb { E } _ { \rho _ { \pi ^ { \theta _ { a } } } } [ \mathrm { l o g } ( 1 - D _ { \phi _ { a } } ( s _ { t } ^ { i } , a _ { t } ^ { i , a g e n t } ) ) ]$ aims to maximize the πθa [log(1 a ( ))]probability that the discriminator correctly recognizes stateaction pairs generated by the generator as not coming from the expert. This drives the generator to develop behaviors that are distinct from those of the expert, fostering differentiation. In summary, the generator is encouraged to replicate expert behavior while also ensuring that the discriminator can effectively distinguish between expert-generated and generator-generated behaviors. The process for updating the discriminator parameters is as follows:

$$
\phi_ {a} \leftarrow \phi_ {a} - l _ {1} * \nabla_ {\phi_ {a}} \mathbb {L} (\phi_ {a}), \tag {24}
$$
...
$$
J \left(\theta_ {a}\right) = \mathbb {E} _ {\left(s _ {t} ^ {i}, a _ {t} ^ {i, a g e n t}\right) \sim \pi^ {\theta_ {a}}} \left[ \log \left(1 - D _ {\phi_ {a}} \left(s _ {t} ^ {i}, a _ {t} ^ {i, a g e n t}\right)\right) \right]. \tag {25}
$$

Accordingly, the parameter update process for the policy network is defined as follows:

$$
\theta_ {a} \leftarrow \theta_ {a} + l _ {2} \cdot \nabla_ {\theta_ {a}} J (\theta_ {a}), \tag {26}
$$
...
$$
\mathbb {L} (\psi_ {a}) = \mathbb {E} \left[ \left(V _ {\psi_ {a}} (s _ {t} ^ {i}) - \hat {R}\right) ^ {2} \right], \tag {27}
$$
...
$$
\psi_ {a} \leftarrow \psi_ {a} - l _ {3} * \nabla_ {\psi_ {a}} \mathbb {L} (\psi_ {a}), \tag {28}
$$
...
$$
\begin{array}{l} \hat {r} \left(s _ {t} ^ {i}, a _ {t} ^ {i, a g e n t}\right) = \log D _ {\phi_ {a}} \left(s _ {t} ^ {i}, a _ {t} ^ {i, e x p e r t}\right) \\ - \log \left(1 - D _ {\phi_ {a}} \left(s - t ^ {i}, a _ {t} ^ {i, a g e n t}\right)\right) \tag {29} \\ \end{array}
$$
...
Time Complexity: In the training phase, ILCTS employs a generator and a discriminator in a GAN-like framework. The generator, trained using IPPO, comprises a policy and value network. The policy network features a hybrid GNN and MLP. The GNN processes UAV domain features with an input and output dimension of 5, U nodes, and U(U−1) $U ( U - 1 )$ 2 edges, yielding a single-layer complexity of $O ( 5 E + 2 5 U )$ and $L _ { 1 } = 6$ layers for a total complexity of $O ( \dot { L } _ { 1 } ( 5 E + 2 5 U ) )$ . = 6The MLP for MU domain features has $L _ { 2 } = 2$ + ))layers, with = 8plexity of $\varsigma _ { 0 } = 8 M$ input and $O ( \sum _ { z = 1 } ^ { L _ { 2 } + 1 } \varsigma _ { z } \cdot \varsigma _ { z - 1 } )$ $\varsigma _ { L _ { 2 } + 1 } = 4$ . A s = 2output neurons, and a com- fuses fea-$L _ { 3 } = 4$ $\varsigma _ { 0 } = 5 U + 4$ $\varsigma _ { L _ { 3 } + 1 } = U + 1$ = 4 =output neurons, with complexity $O ( \sum _ { z = 1 } ^ { L _ { 3 } + 1 } \zeta _ { z }$

Algorithm 2: ILCTS Algorithm.   
Input: Expert trajectories $\Omega_E$ , batch size $N_{batch}$ , policy params $\theta_a$ , value paras $\psi_a$ , discriminator params $\phi_a$ , learning rate $[l_1, l_2, l_3]$ ;

Output: optimal $\pi^{\theta_a}$ ;

1: for $i\_episode = 0, 1, \ldots$ do

2: Initialize environment, $t = 0$ , obtain the state $s_0$ ;

3: while not Done Condition do

4: for $i$ in range( $\mathcal{M}$ ) do

5: Obtain the state $s_t^i$ ;

6: Choose an agent action $a_t^{i,agent}$ based on

7: policy $\pi^{\theta_a}$ ;

8: if A batch of trajectories is sampled then

9: Update $\phi_a$ by gradient;

10: Compute estimated reward $\hat{r}$ ;

11: Update $\psi_a$ by gradient;

12: Update $\theta_a$ using the policy gradient;
...
$\varsigma _ { z - 1 } )$ . The value network mirrors the policy network, shar-)ing a time complexity of O $\begin{array} { r l } { ~ } & { { } ( \sum _ { z = 1 } ^ { L _ { 2 } + 1 } \varsigma _ { z } \cdot \varsigma _ { z - 1 } , L _ { 1 } ( 5 E + } \end{array}$ $\begin{array} { r } { 2 5 U ) ) + \sum _ { z = 1 } ^ { L _ { 3 } + 1 } \varsigma _ { z } \cdot \varsigma _ { z - 1 } \big ) } \end{array}$ (max( z=1   (5 +. The generator’s overall com-)) +plexity is $\begin{array} { r } { O ( K \cdot [ \operatorname* { m a x } ( \sum _ { z = 1 } ^ { L _ { 2 } + 1 } \varsigma _ { z } \cdot \varsigma _ { z - 1 } , L _ { 1 } ( 5 E + 2 5 U ) ) + } \end{array}$ ( [max( (5 + )) +L3+1z=1 ςz · ςz−1 , where K is the number of IPPO updates. The $\begin{array} { r } { \sum _ { z = 1 } ^ { L _ { 3 } + 1 } \varsigma _ { z } \cdot \varsigma _ { z - 1 } \big ] \big ) } \end{array}$ ])discriminator has the same structure and complexity as the policy network. Thus, the total ILCTS complexity is $( \dot { \mathcal { Q } } \cdot T \cdot \dot { N _ { b a t c h } } )$ (times the generator’s complexity, where Q, T , and $N _ { b a t c h }$ )denote the maximum number of episodes, time slots, and batch size, respectively.
...
We employ multi-layer perceptions and a hybrid graph convolutional neural network for imitation learning. The learning

TABLE II RELEVANT PARAMETER SETTINGS: 

<table><tr><td>Parameters</td><td>Value</td></tr><tr><td>Tx Power of UAVs</td><td>20 mW</td></tr><tr><td>CPU Frequency of UAVs</td><td>5G HZ</td></tr><tr><td>UAVs&#x27; Computation Intensity</td><td>15 ~ 20CPU Circles/Byte</td></tr><tr><td>Tx Power of UE</td><td>1.5 mW</td></tr><tr><td>Input Data Size</td><td>3.0 ~ 5.0 MB</td></tr><tr><td>Data Size of Results</td><td>1.5 ~ 2.5 MB</td></tr><tr><td>UAVs&#x27; Communication Radius</td><td>500 Meter</td></tr><tr><td>UAVs&#x27; Flight Altitude</td><td>70 Meter</td></tr><tr><td>Tasks&#x27; Required CPU Cycles</td><td>1.5e8 ~ 2e8 CPU Circles/s</td></tr><tr><td>Gaussian Noise Power  $\sigma^2$ </td><td>-174 dbm/HZ</td></tr><tr><td>Number of Training Episodes</td><td>3000</td></tr><tr><td>Replay Buffer Capacity G</td><td>256</td></tr><tr><td>Batch size  $N_{batch}$ </td><td>256</td></tr><tr><td>Mini-Batch Size</td><td>128</td></tr></table>
...
![](images/6791dadabe164837c912c556abc82634edcb0372f1adb92916660f56c01df258.jpg)

<details>
<summary>line</summary>

| Training Episodes | ILCTS | IPPO | SR-CL | DDQN |
| ----------------- | ----- | ---- | ----- | ---- |
| 0                 | 0.2   | 0.2  | 0.2   | 0.2  |
| 500               | 0.6   | 0.6  | 0.4   | 0.4  |
| 1000              | 0.7   | 0.7  | 0.5   | 0.5  |
| 1500              | 0.7   | 0.7  | 0.6   | 0.6  |
| 2000              | 0.7   | 0.7  | 0.6   | 0.6  |
...
![](images/05d1f4dd5610c8dde3b42a5276df654ed55247bfc76ce0f7579a2c68091252d6.jpg)
...
| 0                 | 0.4   | 0.3  | 0.3   | 0.1  |
| 500               | 0.6   | 0.5  | 0.5   | 0.2  |
| 1000              | 0.7   | 0.6  | 0.6   | 0.3  |
| 1500              | 0.75  | 0.65 | 0.65  | 0.4  |
| 2000              | 0.8   | 0.7  | 0.7   | 0.5  |
| 2500              | 0.8   | 0.7  | 0.7   | 0.55 |
| 3000              | 0.8   | 0.7  | 0.7   | 0.6  |
</details>

(b)MU20

![](images/d02b6078623d3324d755eb4587e8dc79bfda58f51bb5d10af7aa84b5a3cbd30c.jpg)
...
| 0                 | 0.2   | 0.2  | 0.2   | 0.2  |
| 500               | 0.8   | 0.7  | 0.6   | 0.3  |
| 1000              | 0.8   | 0.7  | 0.6   | 0.3  |
| 1500              | 0.8   | 0.7  | 0.6   | 0.3  |
| 2000              | 0.8   | 0.7  | 0.6   | 0.3  |
| 2500              | 0.8   | 0.7  | 0.6   | 0.3  |
| 3000              | 0.8   | 0.7  | 0.6   | 0.3  |
</details>

(c)MU30

![](images/1e991feb3a645594ac5baa9c6223d9c0c81995c331e84c23384dca7e43197605.jpg)
...
| 0                 | 0.4   | 0.4  | 0.4   | 0.4  |
| 500               | 0.6   | 0.5  | 0.5   | 0.3  |
| 1000              | 0.7   | 0.6  | 0.6   | 0.4  |
| 1500              | 0.7   | 0.6  | 0.6   | 0.4  |
| 2000              | 0.7   | 0.6  | 0.6   | 0.4  |
| 2500              | 0.7   | 0.6  | 0.6   | 0.4  |
| 3000              | 0.7   | 0.6  | 0.6   | 0.4  |
</details>

(d)MU40

Fig. 5. Training performance with different MUs.   
![](images/26fdb3ca2d363890e493ba40135baed56754efff73416cb54711e75b4f808357.jpg)

<details>
<summary>line</summary>

| Training Episodes | Without Sliding Window | With Sliding Window |
| ----------------- | ---------------------- | ------------------- |
| 0                 | 0.2                    | 0.1                 |
| 500               | 0.6                    | 0.4                 |
| 1000              | 0.65                   | 0.6                 |
...
![](images/adcee53d8eb12dd50d541b9224815a4a3c08a7abbf77db52a72923621fe30aea.jpg)

<details>
<summary>bar</summary>

| Number of MUs | GBLM  | OLSA  | DDQN  | SR-CL | IPPO  | ILCTS |
| ------------- | ----- | ----- | ----- | ----- | ----- | ----- |
| 10            | 40    | 100   | 50    | 60    | 60    | 60    |
| 15            | 50    | 130   | 60    | 60    | 60    | 60    |
| 20            | 70    | 150   | 70    | 70    | 70    | 70    |
| 30            | 80    | 180   | 80    | 80    | 80    | 80    |
| 80            | 120   | 350   | 110   | 110   | 110   | 110   |
</details>
...
![](images/bf75dcc0121c3851552c10a98172fb13d35897906d800f38f012a33cef29c3cb.jpg)

<details>
<summary>line</summary>

| Training Episodes | ReLU  | Tanh  | LReLU |
| ----------------- | ----- | ----- | ----- |
| 0                 | 0.4   | 0.4   | 0.4   |
| 500               | 0.6   | 0.5   | 0.5   |
| 1000              | 0.7   | 0.6   | 0.6   |
...
![](images/fac9cbf1b7dd12363f71872a787c98f7f6a17280fe9bd4b1111d95b06f87d092.jpg)

<details>
<summary>line</summary>

| Training Episodes | Orthogonal | He    | Xavier |
| ----------------- | ---------- | ----- | ------ |
| 0                 | 0.2        | 0.2   | 0.2    |
| 500               | 0.6        | 0.6   | 0.5    |
| 1000              | 0.7        | 0.65  | 0.6    |
...
![](images/9d310341cc145ece3b84efd654f0dd1460dc11993d637e5818012ad91fbaf851.jpg)

<details>
<summary>line</summary>

| Training Episodes | GCN   | MLP   | GMM   | GMM+GCN |
| ----------------- | ----- | ----- | ----- | ------- |
| 0                 | 0.2   | 0.2   | 0.2   | 0.2     |
| 500               | 0.6   | 0.5   | 0.5   | 0.7     |
| 1000              | 0.7   | 0.6   | 0.6   | 0.8     |
| 1500              | 0.75  | 0.65  | 0.65  | 0.85    |
| 2000              | 0.8   | 0.7   | 0.7   | 0.9     |
| 2500              | 0.85  | 0.75  | 0.75  | 0.95    |
| 3000              | 0.9   | 0.8   | 0.8   | 1.0     |
</details>

(c) Neural Network Architecture

![](images/5b470ac4d0af1a47cb0828491c39d87ddee24724e67b70a3324983a91dbef938.jpg)

<details>
<summary>line</summary>

| Training Episodes | Adam  | NAdam | SGD   |
| ----------------- | ----- | ----- | ----- |
| 0                 | 0.4   | 0.4   | 0.35  |
| 500               | 0.6   | 0.55  | 0.38  |
| 1000              | 0.75  | 0.65  | 0.4   |
...
![](images/0207839f94b9b931996d4e7fce43b6f0154e20ddb76c5cecb5b47860c740b8fa.jpg)
...
| Number of MUs | GBLM  | OLSA  | DDQN  | SR-CL | IPPO  | ILCTS |
| ------------- | ----- | ----- | ----- | ----- | ----- | ----- |
| 10            | 840   | 840   | 760   | 750   | 750   | 720   |
| 15            | 760   | 760   | 720   | 720   | 720   | 700   |
| 20            | 760   | 760   | 760   | 720   | 720   | 710   |
| 30            | 690   | 690   | 680   | 670   | 670   | 650   |
| 80            | 720   | 720   | 680   | 650   | 650   | 630   |
</details>

![](images/0e8baa933da4eb08c13d83c3a146c6f46a048cb6175466dbcd504efbf1f55fa1.jpg)
...
![](images/dce8e1898a0fe49abc9926ddad53d7d5f95bb419e5c49d448b4ad5bcade4b331.jpg)

<details>
<summary>bar_stacked</summary>

(a) Hard delay tasks
| Task Index | Download Latency (ms) | Migration Latency (ms) |
| :--- | :--- | :--- |
| 1 | 800 | 200 |
| 2 | 700 | 150 |
| 3 | 700 | 150 |
...
![](images/686c89ab75e51b0b3674dfcae05688003d5a91af54c7ac5e366dfbf570e320bc.jpg)

<details>
<summary>bar_stacked</summary>

(b) Soft delay tasks
| Task Index | Computation Latency (ms) | Uploading Latency (ms) |
| :--- | :--- | :--- |
| 1 | 500 | 70 |
| 2 | 480 | 100 |
| 3 | 460 | 150 |
...
![](images/b38e5caec6babe9a7a9514941a07b5e83ebb668a17ff16237808fd6e8fcecb25.jpg)

<details>
<summary>bar</summary>

(a) Mean
| Category | GBLM (ms) | OLSA (ms) | DDQN (ms) |
| :--- | :--- | :--- | :--- |
| mean = 50 | 868 | 862 | 804 |
| mean = 60 | 874 | 860 | 804 |
| mean = 70 | 859 | 859 | 773 |
...
![](images/080d5e99c6a7d8474437f8c93f6ccaa292b29b1414a21b37cc229be0423fbb4d.jpg)

<details>
<summary>bar</summary>

| std | SR-CL | IPPO | ILCTS |
| --- | --- | --- | --- |
| 10 | 820 | 790 | 700 |
| 15 | 860 | 790 | 750 |
| 25 | 860 | 840 | 720 |
...
![](images/98c9530e29c87a9af70242e233fec286022f03f02de5686d658881d2d4affa35.jpg)

<details>
<summary>bar</summary>

(a) Arrival Rate Parameter λ
| λ | GBLM (ms) | OLSA (ms) | DDQN (ms) |
|---|---|---|---|
| λ = 1.0 | 810 | 795 | 785 |
| λ = 1.6 | 815 | 800 | 765 |
| λ = 4.0 | 860 | 840 | 780 |
...
![](images/4db3782c3af5c0a9ce511347d62c5d60dcc6930bbf7ef61adb0cfab301892a69.jpg)

<details>
<summary>bar</summary>

(b) Threshold for Execution Progress (%)
| Threshold for Execution Progress (%) | SR-CL (ms) | IPPO (ms) | ILCTS (ms) |
| :--- | :--- | :--- | :--- |
| 0.4 | 765 | 760 | 700 |
| 0.5 | 775 | 750 | 720 |
| 0.6 | 785 | 765 | 735 |
...
![](images/cc00b07f6a18e9ea3a421ae98271f12a3d6e8e412faece5de4e523e49cde19a1.jpg)

<details>
<summary>bar</summary>

| Mobility Model | GBLM  | OLSA  | DDQN  | SR-CL | IPPO  | ILCTS |
| -------------- | ----- | ----- | ----- | ----- | ----- | ----- |
| RWP            | 850   | 840   | 760   | 750   | 740   | 720   |
| GeoLife        | 950   | 880   | 840   | 830   | 820   | 760   |
| HMM            | 880   | 860   | 800   | 740   | 750   | 680   |
</details>
...
![](images/add4672f54f994dab8dbc8c7b47e988d73e4900fec4cd4aba190df086836facb.jpg)

<details>
<summary>natural_image</summary>

Portrait of a man in a suit against a red background (no text or symbols visible)
</details>
...
![](images/cc45c38ee0ad1a7f6b89db59f23a7644bb06f3a7213e8f76db2fae31cc21b472.jpg)

<details>
<summary>natural_image</summary>

Portrait of a young man wearing glasses and a patterned cardigan (no text or symbols visible)
</details>
...
![](images/84b7062578ca630c3b33177fe0bd5c78df1cb5df16dfac1c42f287b2d96c4255.jpg)
...
![](images/0c7e82766b87e76d0feb6a211383904d3aecd3162372e99b8741d36d57d62826.jpg)
...
![](images/a9930361de30c305b3c127bb13b1580226f257361b9735af271c64d3ee75a3c5.jpg)
...
![](images/9246d3b00558d23677df57ea830550d1c92182c19991fecab5844fa1ddbb04e5.jpg)
...
![](images/6adb71f6a850498a396631b011c5265e668140db3b9c958a135870c56fa643e0.jpg)
