---
type: entity
title: Tony Q. S. Quek
tags: [author, researcher, ieee-fellow, edge-computing, sutd]
related:
  - "[[liu-2025-haps-uav-maritime-iot]]"
  - "[[yao-2025-secure-isac-dual-eavesdropping]]"
  - "[[zhao-2024-caching-service-placement-uav]]"
created: 2026-05-29
updated: 2026-05-29
---

# Tony Q. S. Quek

IEEE Fellow. Affiliated with the **Information Systems Technology and Design (ISTD) Pillar, Singapore University of Technology and Design (SUTD)** (Singapore; email `tonyquek@sutd.edu.sg`); some papers also list Yonsei University / Kyung Hee University. Consistent identity across his papers in this wiki.

Co-authored 3 sources in this wiki:

- [[liu-2025-haps-uav-maritime-iot]] — HAP-UAV-assisted maritime IoT communication network.
- [[yao-2025-secure-isac-dual-eavesdropping]] — UAV-enabled secure ISAC against dual eavesdropping threats.
- [[zhao-2024-caching-service-placement-uav]] — joint content caching, service placement, and task offloading in UAV-enabled MEC.

## Contributions to this wiki

- A senior collaborator (edge computing, ISAC, wireless networks) recurring across the maritime-IoT, secure-ISAC, and UAV-caching threads of the corpus.
