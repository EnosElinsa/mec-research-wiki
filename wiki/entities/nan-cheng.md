---
type: entity
title: Nan Cheng
tags: [author, researcher, ieee-senior, non-terrestrial-networks, uav-isac]
related:
  - "[[lyu-2026-situation-aware-uav-isac]]"
  - "[[cheng-2019-sagin-iot-offloading-rl]]"
  - "[[li-2020-energy-efficient-uav-mec-admm]]"
  - "[[zhou-2021-delay-sagin-task-scheduling]]"
  - "[[dai-2024-graph-rm-survey-optimization]]"
  - "[[dai-2024-graph-rm-survey-learning]]"
  - "[[ling-lyu]]"
  - "[[yanpeng-dai]]"
created: 2026-07-14
updated: 2026-07-15
---

# Nan Cheng

IEEE Senior Member. Affiliated with the **State Key Laboratory of Integrated Services Networks** and the **School of Telecommunications Engineering, Xidian University**; the current PDF gives `dr.nan.cheng@ieee.org`. Exact-name, Xidian-affiliation, and repeated Ling Lyu/Yanpeng Dai/Xuemin Shen coauthorship establish the identity represented here.

Co-authored six affiliation-confirmed sources in this wiki:

- [[lyu-2026-situation-aware-uav-isac]] - situation-aware sensing and position control for UAV-assisted ISAC.
- [[cheng-2019-sagin-iot-offloading-rl]] - learning-based space/aerial computing offloading for IoT applications.
- [[li-2020-energy-efficient-uav-mec-admm]] - energy-efficient UAV-MEC resource allocation and trajectory optimization.
- [[zhou-2021-delay-sagin-task-scheduling]] - delay-oriented IoT task scheduling in SAGIN.
- [[dai-2024-graph-rm-survey-optimization]] - graph optimization for wireless resource management.
- [[dai-2024-graph-rm-survey-learning]] - graph learning for wireless resource management.

## Contributions to this wiki

- Connects early SAGIN and UAV-MEC resource management to graph-based wireless optimization and UAV-assisted industrial sensing/control.

## Verified coauthor links

The [[lyu-2026-situation-aware-uav-isac]] byline directly establishes coauthorship with:

- [[ling-lyu]]
- [[yanpeng-dai]]
