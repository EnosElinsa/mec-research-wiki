...
Raw review accepted **18** direct frozen-core relationships. The AirComp, critical-period, simultaneous-interference, contract, and tree-aggregation groups were joined through topology-, timing-, access-, statistic-, eligibility-, and aggregation-pipeline contrasts. Six candidate pairs remained rejected: direct Dang links to FDN/FKN, Li to aggregation MSE, aggregation MSE to simultaneous-interference FL, Lim directly to Li, and tree weight synthesis to gradient-correlation MSE. Those pairs were indirect, redundant, or liable to conflate incompatible metrics; the narrower accepted concept links preserve their useful distinctions.

The frozen Research Log core gained **18** induced edges in this theme, reduced components from **42 to 38** (**-4**), and reduced weak members from **84 to 81** (**-3**), with no removed edge. Cumulatively from the 341-edge baseline, the graph now has **400** edges (**+59**), **38** components (**-9**), and **81** weak members (**-6**). [[dang-2026-uav-fl-energy]] rose from internal degree **1 to 6**, and [[simultaneous-interference-uav-federated-learning]] from **1 to 4**. Commit scope is limited to the new synthesis, nine source pages, twelve concept pages, and `index.md`, `overview.md`, and this log; the evidence matrix, graph reports, and coverage ledger remain scratch artifacts.

## [2026-07-14] Synthesized constraint regimes in UAV data collection

Added [[constraint-regimes-in-uav-data-collection]] as a reader path across [[zhu-2023-aoi-transformer-trajectory]], [[samir-2020-time-constrained-data-collection]], [[chang-2026-data-offloading-energy-constraints]], [[qi-2026-ocma-ddqn-data-collection]], [[zhao-2026-uav-carrier-vcs]], [[li-2023-energy-constrained-uav-data-collection]], [[fu-2026-dubins-uav-data-collection]], and [[guo-2026-spatiotemporal-information-quality-ugrnet]]. The synthesis separates hard feasibility constraints from weighted reward/surrogate terms and keeps collected bits, AoI, served-device count, completion time, energy, data ratio, fairness, delay-violation probability, and Wasserstein spatial mismatch as distinct outputs.

Raw review accepted 20 direct frozen-core relationships: AoI/GTSP was joined to deadline/energy routing; learned search was contrasted with tolerance-qualified BRB and exact MILP formulation without transferring guarantees; deadline, precedence, and fixed-order replenishment were separated; and the interruption-aware multi-UAV component was joined to the carrier-enabled UAV/UGV component through source- and mechanism-level links. Four tempting pairs remained rejected because they offered only an indirect taxonomy contrast: pickup precedence versus opportunistic exchange, map/experience exchange versus hidden-state sharing, distance-value circles versus policy-divergence exploration, and LSTM action imputation versus hidden-state sharing.

The frozen Research Log core gained **20** induced edges in this theme, reduced components from **45 to 42** (**-3**), and reduced weak members from **85 to 84** (**-1**). Cumulatively from the 341-edge frozen baseline, the graph now has **382** edges (**+41**), **42** components (**-5**), and **84** weak members (**-3**), with no removed edge. [[mixed-integer-linear-programming]] now has frozen-core degree **4**, up from baseline degree 1. The commit scope is limited to the new synthesis, the eight primary source pages, eighteen allowed concept pages, and `index.md`, `overview.md`, and this log; the raw-grounding matrix, graph reports, and coverage ledger remain scratch artifacts.
...
- [[yan-2026-uav-trajectory-monitoring]] - Yan et al. 2026, *IEEE TWC* 25, 2733-2747, DOI `10.1109/TWC.2025.3598799`. [[uav-trajectory-monitoring]], [[phase-rotated-dft-motion-parameter-estimation]], and [[position-gated-velocity-nearest-neighbor-association]] separate target-UAV sensing, spectral motion estimation, and crossing-trajectory association.
- [[zhu-2023-aoi-transformer-trajectory]] - Zhu et al. 2023, *IEEE TWC* 22(2), 1343-1358, DOI `10.1109/TWC.2022.3204438`. [[transformer-weighted-a-star-trajectory-planning]], [[generalized-traveling-salesman-problem]], and [[hovering-disk-data-collection]] decompose cluster ordering from SNR-feasible contact-point selection.
- [[samir-2020-time-constrained-data-collection]] - Samir et al. 2020, *IEEE TWC* 19(1), 34-46, DOI `10.1109/TWC.2019.2940447`. [[deadline-constrained-uav-data-collection]] and [[branch-reduce-and-bound]] distinguish hard-lifetime admission and a small-instance global benchmark from scalable SCA.
- [[chang-2026-data-offloading-energy-constraints]] - Chang et al. 2026, *IEEE TGCN* 10, 573-584, DOI `10.1109/TGCN.2025.3592263`. [[many-to-one-pickup-and-delivery]], [[dynamic-programming-battery-station-insertion]], and [[mixed-integer-linear-programming]] organize mandatory IoT pickup, predetermined edge-server delivery, and battery replacement.
- [[hua-2026-unpredictable-uav-trajectory]] - Hua et al. 2026, *IEEE T-ITS* 27(1), 448-459, DOI `10.1109/TITS.2025.3605584`. [[unpredictable-uav-trajectory-control]] and [[navigation-stochastic-control-decomposition]] combine finite-horizon mission guidance with stochastic heading perturbations.

Evidence caveats: the ISAC monitoring results are simulation-only and do not guarantee association or complete tracking for arbitrary targets. The AoI policy is trained on generated 10-cluster instances, discretizes continuous hover disks without an error bound, and has no full-pipeline optimality or general distribution-shift guarantee. BRB's global claim is tolerance-qualified for the formulated small problem, while both SCA variants remain local and online CSI repair cannot guarantee every fading realization. The pickup/delivery dynamic program is optimal only for a fixed service order under idealized rates and battery replacement; the complete TMTP heuristic has no approximation ratio. The unpredictable-trajectory study assumes exact current jammer locations, evaluates one-step Kalman prediction error against nonadaptive baselines, and does not establish security against method-aware attackers; its Gaussian control law also lacks a visible clipping rule despite finite stated bounds.

Exact-title Crossref records and local PDF headers supplied DOI, venue, issue-year, and pagination omitted from the Markdown parses. Online-publication dates were kept distinct from final issue years for the 2026 monitoring and pickup/delivery articles. Identity review used repeated biographies, affiliations, emails, and coauthor continuity to establish [[shaoqiang-yan]], [[hongliang-luo]], [[ping-yang]], [[feifei-gao]], [[moataz-samir]], [[sanaa-sharafeddine]], [[chadi-assi]], and [[ali-ghrayeb]]; future Ping Yang matches require corroborating institutional or coauthor evidence because of name collision risk.

The integrated corpus contains **609 sources / 648 concepts / 185 entities / 612 raw folders**, leaving **2 genuinely new sources** for the final curation batch. Independent review corrected the Gaussian-control boundedness claim, narrowed the index's prediction language, and added two online/final issue chronologies. Wikilinks and process narration are clean; the index covers all **1,494** catalogue-able pages exactly once; all **1,492** typed frontmatter blocks validate; entity rosters have zero overclaims (20 advisory omissions/namesakes remain); all 12 wiki-tool unit tests pass; and `git diff --check` is clean. The optional local LLM Wiki health endpoint returns HTTP **502**, so file-grounded CLI audits remain authoritative.

## [2026-07-14] Curated underground wireless power, UAV-WPT tutorial structure, secure maritime coexistence, cellular U2U, and carrier-enabled crowdsensing

Added five source pages and twelve reusable concepts:

- [[lin-2026-uav-wpucn-time-allocation]] - Lin et al. 2026, *IEEE TGCN* 10, 1618-1632, DOI `10.1109/TGCN.2025.3644128`. [[wireless-powered-underground-communication-network]], [[underground-air-soil-wireless-channel]], and [[csi-free-multiantenna-wireless-energy-transfer]] separate the soil-aware architecture, propagation model, and practical WET alternatives from the fixed-geometry convex time-allocation result.
- [[xie-2021-uav-wpt-tutorial]] - Xie et al. 2021, *IEEE TGCN* 5(4), 2042-2064, DOI `10.1109/TGCN.2021.3093718`. [[multi-location-hovering]], [[wireless-powered-communication-network]], and [[computation-causality-constraint]] organize relaxed trajectory structure, harvest-then-transmit communication, and the unsolved full wireless-powered-MEC formulation.
- [[yao-2026-secure-maritime-sutn]] - Yao et al. 2026, *IEEE TWC* 25, 2342-2358, DOI `10.1109/TWC.2025.3596136`. [[norm-bounded-csi-robust-optimization]] and [[s-procedure-for-csi-uncertainty]] connect deterministic channel-error balls to finite beamforming LMIs in satellite-UAV-terrestrial maritime coexistence.
- [[azari-2020-uav-to-uav-cellular]] - Azari et al. 2020, *IEEE TWC* 19(9), 6130-6144, DOI `10.1109/TWC.2020.3000303`. [[uav-to-uav-communication]] and [[fractional-power-control]] connect direct aerial links to stochastic-geometry cellular-uplink coexistence; the source explicitly uses overlay to mean orthogonal bandwidth partition.
- [[zhao-2026-uav-carrier-vcs]] - Zhao et al. 2026, *IEEE TMC*, 1-18, DOI `10.1109/TMC.2026.3693470`. [[attentive-memory-integrated-information-exchange]] and [[mutual-policy-divergence-exploration]] coordinate unequal-duration UAV/UGV decisions through message memory, ordered actions, role differentiation, and continued exploration.

Evidence caveats: the underground-network optimizer fixes route, speed, altitude, and hover point and has no finite mission deadline inside its feasibility claim; its parse also contains threshold/unit and charging-time inconsistencies. The tutorial's speed-relaxed hovering solutions are upper bounds for finite-speed missions, generic multi-UAV WPT remains open, and the full computation-causality-constrained MEC formulation is not solved. The maritime design handles its declared norm-bounded uncertainty sets through local AO/SDP/SCA blocks without a global joint optimum or robustness beyond those sets. The cellular-U2U formulas are exact only under the stated PPP, channel, antenna, association, and power-control model, while their compact corollaries add explicit approximations. The crowdsensing evaluation uses real map geometry with synthetic PoIs/demands and supplies empirical convergence and loss/delay stress tests rather than theoretical, safety, or sim-to-real guarantees.

Three low-concurrency extraction agents grounded disjoint parses and two workers drafted disjoint source sets. Exact-title Crossref records supplied final DOI, year, venue, volume, issue, and page metadata omitted from the title-named parses; the 2025 online dates of the underground and maritime papers were reconciled to their final 2026 volume records. Existing author rosters were extended only for confirmed [[jie-xu]], [[chan-byoung-chae]], [[kai-kit-wong]], and [[chi-harold-liu]] identities; initials-only and biography-only candidates were not promoted.

Validation reports **604 sources / 635 concepts / 177 entities / 612 raw folders** and **7 genuinely new sources** remaining. Independent review corrected the interweave/cognitive-overlay/cellular-overlay taxonomy, qualified S-procedure equivalence, documented the maritime paper's online/final-volume chronology, and completed three direct specialization backlinks. Wikilinks and process narration are clean; the index covers all **1,468** catalogue-able pages exactly once; all **1,466** typed frontmatter blocks validate; entity rosters have zero overclaims (20 advisory omissions/namesakes remain); all 12 wiki-tool unit tests pass; and `git diff --check` is clean. The optional local LLM Wiki health endpoint returns HTTP **502**, so file-grounded CLI audits remain authoritative.

## [2026-07-14] Curated cooperative covert radio, FC-RIS surveillance, STAR-RIS monitoring, hierarchical OTA-FL, and passive 6D arrays

Added five source pages, ten reusable concepts, and seven recurring-author entities:

- [[wang-2026-covert-cognitive-radio]] - Wang et al. 2026, *IEEE TWC* 25, 1594-1608, DOI `10.1109/TWC.2025.3591810`. [[cooperative-cognitive-radio]] and [[primary-signal-assisted-covertness]] separate useful primary forwarding from dedicated cover traffic or jamming.
- [[lin-2026-fc-ris-surveillance]] - Lin et al. 2026, *IEEE TWC* 25, 6160-6176, DOI `10.1109/TWC.2025.3623963`. [[wireless-information-surveillance]], [[monitoring-success-probability]], [[fully-connected-ris]], and [[threshold-based-antenna-selection]] connect legitimate decoding to partial-CSI receiver/surface control.
- [[zhan-2026-star-ris-aerial-monitoring]] - Zhan et al. 2026, *IEEE TWC* 25, 8769-8783, DOI `10.1109/TWC.2025.3645801`. A Lyapunov energy queue, WMMSE/PDD beamforming, STAR-RIS coefficients, and SCA trajectory updates coordinate stochastic target monitoring and indoor/outdoor dissemination.
- [[zhong-2026-hierarchical-ota-fl]] - Zhong et al. 2026, *IEEE TWC* 25, 8066-8082, DOI `10.1109/TWC.2025.3635287`. [[hierarchical-over-the-air-federated-learning]] and [[gradient-correlation-aware-aggregation-mse]] connect trajectory-segmented partial aggregates to a learning-stationarity bound.
- [[liu-2026-passive-6dma]] - Liu et al. 2026, *IEEE TWC* 25, 9765-9781, DOI `10.1109/TWC.2025.3643647`. [[passive-six-dimensional-movable-antenna]] and [[angle-dependent-irs-effective-aperture]] distinguish rigid UAV-mounted reflecting surfaces from active arrays and element-level movable antennas.

Biography, affiliation, email, degree-history, ORCID, and coauthor continuity established [[qunshu-wang]] across two covert-UAV sources; [[cheng-zhan]] across four UAV data-collection, MEC, offloading, and monitoring sources; [[kaifeng-song]] and [[rongfei-fan]] across two BIT edge-intelligence sources each; [[han-hu]] across three UAV-MEC/monitoring sources; [[xiaojun-yuan]] across robust RIS security and hierarchical OTA-FL; and [[ying-jun-angela-zhang]] across stochastic UAV-MEC and OTA-FL. Existing rosters were extended to 66 sources for [[dusit-niyato]], ten for [[jie-xu]], eight for [[derrick-wing-kwan-ng]], and three for [[chengwen-xing]]. Nan Zhao and Bin Li were not promoted because the corpus contains institutionally distinct or high-collision exact-name records.

Evidence caveats: the cognitive-radio design uses a conservative Pinsker/KL covertness condition and locally convergent BCD/SCA, with primary-only forwarding under the detector's null hypothesis; it omits propulsion energy and assumes known wardens. FC-RIS surveillance relies on ideal symmetric-unitary hardware, integer Nakagami parameters, an approximate suspicious-channel distribution, and fixed-altitude/equal-exponent placement analysis; its global channel-gain statement applies only after one antenna is selected. STAR-RIS monitoring makes queue-based energy feasibility conditional on stability and has no global result for the full stochastic AO problem. Hierarchical OTA-FL bounds stationarity by aggregation MSE under smoothness/correlation assumptions, while relaxed device selection, local convexification, synchronization, and error-free side channels limit deployment claims. Passive 6D analysis proves one-axis orientation sufficiency only for a restricted single-user geometry; the general three-user solver uses finite grids, structured phases, local AO, and Gibbs exploration without a global or discretization guarantee. All five evaluations are simulation-only.

Three low-concurrency extraction agents grounded the five title-named parses, two disjoint workers drafted source pages, and an independent read-only reviewer checked metadata, theorem scope, concepts, author identity, and graph integration. Exact-title Crossref records supplied final 2026 TWC volume/page metadata omitted from four parses and reconciled the hierarchical OTA-FL article's 2025 online date with its 2026 final volume. Review corrected the covert detector's null-hypothesis wording and expanded two raw-artifact blocks to explicit parse, PDF, and figure paths.

Validation reports **599 sources / 623 concepts / 177 entities / 612 raw folders** and **12 genuinely new sources** remaining. Wikilinks are clean; process narration is absent outside this log; the index covers all **1,451** catalogue-able pages exactly once; all **1,449** frontmatter blocks validate; entity rosters have zero overclaims (20 advisory omissions/namesakes remain); all 12 wiki-tool unit tests pass; and `git diff --check` is clean. The optional local LLM Wiki health endpoint is unreachable, so file-grounded CLI audits remain authoritative.
...
- [[xiao-2020-secrecy-energy-efficiency-relaying]] - Xiao et al. 2020, *IEEE TGCN* 4(1), 180-193, DOI `10.1109/TGCN.2019.2949802`. [[collect-store-forward-relaying]] couples secrecy information causality with fixed-wing propulsion energy, scheduling, power, and trajectory optimization.
- [[guo-2026-irs-uav-isac-secrecy]] - Guo et al. 2026, *IEEE TGCN* 10, 289-299, DOI `10.1109/TGCN.2025.3578453`. [[secure-irs-uav-isac]] and [[artificial-noise-aided-physical-layer-security]] combine radar-SNR constraints, mobile reflecting phases, active beams, and bounded-CSI/angle uncertainty.
- [[li-2026-credit-aware-uav-irs-secrecy]] - Li et al. 2026, *IEEE TWC* 25, 3186-3197, DOI `10.1109/TWC.2025.3602188`. [[shapley-value-marl-credit-assignment]] supplies exact coalition rewards, while [[primal-dual-constrained-marl]] discourages non-cooperative UAV-IRS behavior.
- [[huang-2026-intelligent-jamming-maritime]] - Huang et al. 2026, *IEEE TMC* accepted author version, DOI `10.1109/TMC.2026.3688701`. [[advantage-conditioned-cvae-policy]] and [[lstm-eavesdropper-trajectory-prediction]] coordinate relay and jammer UAVs under partial eavesdropper observability.
- [[feng-2026-secure-short-packet-noma-relay]] - Feng et al. 2026, *IEEE TWC* 25, 7635-7648, DOI `10.1109/TWC.2025.3632873`. [[weighted-effective-secrecy-rate]] and [[dual-phase-artificial-noise-uav-relaying]] couple finite-blocklength reliability, NOMA allocation, artificial-noise shares, and hovering relay placement.

Matching biographies, affiliations, emails, and coauthor continuity established [[lin-xiao]], [[yu-xu]], [[dingcheng-yang]], [[xulong-li]], [[wei-huangfu]], [[jiahao-huo]], [[huabing-lu]], [[chengwen-xing]], and [[xianbin-wang]] across two to five sources each. Existing [[yong-zeng]], [[haijun-zhang]], [[jiawei-huang]], [[aimin-wang]], [[geng-sun]], [[jiahui-li]], [[jiacheng-wang]], [[weijie-yuan]], and [[dusit-niyato]] rosters were reconciled against source frontmatter and raw biographies. Nan Zhao remains deferred because the corpus contains institutionally distinct exact-name records; the exact-name Haijun Zhang record in the HAP accepted version remains advisory because that supplied PDF omits affiliation and biography evidence.

Metadata notes: the first, second, third, and fifth PDFs provide final journal headers and page ranges. Their online-publication/copyright years precede the final 2026 volume assignment where applicable, so citation years follow the journal headers. The maritime-jamming source remains an accepted TMC author version with DOI and 2026 copyright but no final volume, issue, or journal page range; manuscript pages 1-18 were not promoted as final pagination.

Evidence caveats: the fixed-wing relay assumes global CSI, perfect Doppler compensation, one passive eavesdropper unable to combine both hops, propulsion-dominant energy, and a disk-uncertainty extension whose solver details are omitted; the IRS-UAV ISAC design assumes continuous unit-amplitude phases, fixed-altitude motion, clutter-free sensing, centralized channel knowledge, a scalar AN model, and local AO/SCA solutions, while its displayed formulation omits a claimed user-SINR constraint; PD-CMASAC uses factorial exact Shapley enumeration, fixed BS precoding/power, ideal IRS coefficients, no flight energy or collision constraints, and reports imperfect-CSI sensitivity rather than a worst-case robust guarantee; maritime SAC-CVAE is simulation-only, trains against the same Gauss-Markov Eve family used for evaluation, leaves prediction/CVAE details unavailable, and cites appendices absent from the supplied accepted PDF; short-packet NOMA assumes known channels, perfect SIC and spatial suppression, one fixed hover point, no propulsion cost, and contains printed baseline, stopping-condition, unit, and integer-blocklength inconsistencies.

Three low-concurrency extraction agents grounded the five papers, two disjoint workers drafted source pages, and one worker drafted concepts. Independent read-only review corrected Phase-I artificial-noise geometry, distinguished cumulative sum secure rate from learning reward, removed a false worst-case-robust classification, reconciled four legacy author rosters, completed reciprocal links, and refreshed every modified frontmatter date.

Validation reports **574 sources / 579 concepts / 159 entities / 612 raw folders** and **37 genuinely new sources** remaining. Wikilinks are clean; process narration is absent outside this log; the index covers all **1,364** catalogue-able pages exactly once; all **1,362** frontmatter blocks validate; entity rosters have zero overclaims (28 advisory omissions/namesakes remain); all 12 wiki-tool unit tests pass; and `git diff --check` is clean. The optional local LLM Wiki health endpoint is unreachable, so file-grounded CLI audits remain authoritative.

## [2026-07-14] Curated optical SLIPT, near-field STAR-RIS ISAC, scalable radio mapping, LLM-guided relaying, and reconfigurable MARL

Added five source pages, fifteen reusable concepts, and five recurring-author entities:

- [[kamatchi-2025-slipt-uav-fso]] - Kamatchi et al. 2025, *IEEE TGCN* 9(3), 1268-1279, DOI `10.1109/TGCN.2024.3514689`. [[simultaneous-lightwave-information-and-power-transfer]], [[ground-to-uav-fso-channel]], and [[fov-aware-optical-uav-reception]] distinguish AC-DC separation, time switching, power splitting, and hybrid TSPS reception under Malaga turbulence, pointing error, and field-of-view loss.
- [[huang-2026-star-ris-nearfield-isac]] - Huang et al. 2026, *IEEE TWC* 25, 3260-3271, DOI `10.1109/TWC.2025.3602989`. [[semi-passive-star-ris]], [[near-field-star-ris-isac]], and [[radar-mutual-information-rate]] connect a transmission-side sensing array, spherical-wave channels, and joint communication/sensing optimization.
- [[chakraborty-2026-skyscale-rti-deployment]] - Chakraborty et al. 2026, *IEEE TMC* 25(7), 9707-9720, DOI `10.1109/TMC.2026.3656779`. SKYSCALE combines [[radio-tomographic-attenuation-mapping]], [[rank-saturation-rem-updates]], and [[segment-coverage-uav-trajectory]] to reduce repeated deployment measurements, including validation on a real WiFi/UAV testbed.
- [[xu-2026-mrlmn-llm-multihop]] - Xu et al. 2026, *IEEE TMC* 25(7), 11173-11190, DOI `10.1109/TMC.2026.3669346`. [[task-oriented-grouped-uav-marl]], [[connectivity-preserving-uav-behavioral-loss]], [[multi-hop-uav-emergency-networking]], and [[llm-guided-marl-policy-distillation]] combine fixed role grouping, reactive link recovery, and offline GPT-4o advice whose distilled policy no longer queries the LLM at deployment.
- [[kim-2026-scale-reconfigurable-marl]] - Kim et al. 2026, *IEEE TMC* accepted author version, DOI `10.1109/TMC.2026.3709180`. [[scale-reconfigurable-marl]] masks a fixed maximum network to match changing visible CubeSat/UAV sets, while [[hidden-state-sharing-marl]] exchanges intermediate feed-forward representations among four ground-station agents.

Exact-name, affiliation, and coauthor continuity established [[xinlei-chen]] across the MRLMN and RMADDPG-DDA papers and established [[gyu-seon-kim]], [[soyi-jung]], [[soohyun-park]], and [[joongheon-kim]] across the scale-reconfigurable and QMARL SAGIN papers. Existing [[zehui-xiong]] and [[dusit-niyato]] rosters were extended to 13 and 60 sources. Nan Zhao and references-only author recurrences were not merged or promoted without sufficient in-corpus identity evidence.

Metadata notes: embedded IEEE PDF headers supplied the final fields omitted from the parses. The scale-reconfigurable paper remains an accepted author version with 2026 copyright, DOI, and manuscript pages 1-18; `PP/99` placeholders were not promoted to final volume, issue, or pagination. The SLIPT title uses `SAGNET`, while the paper body describes a SAGIN setting, so the source page preserves that inconsistency rather than silently normalizing it.

Evidence caveats: SLIPT studies one quasi-static hovering optical link, omits propulsion and hardware validation, scales TS/TSPS error by information duration, and contains a figure-text harvested-energy mismatch; the STAR-RIS study uses one fixed-altitude hover point, continuous coefficients, perfect channel/location knowledge, and no hardware or runtime evidence; SKYSCALE assumes known user locations, persistent segment attenuation, and benefits that depend on mobility/churn and rank growth, with its earlier conference lineage retained; MRLMN is simulation-only, omits energy, interference, signaling, altitude, and collision costs, does not isolate LLM-specific value against a handcrafted advisor, and leaves prompts/verifier thresholds unavailable; scale-reconfigurable MARL fixes four ground stations, uses deterministic device ordering rather than permutation invariance, reports no seed/error bars, and evaluates six CubeSats plus six large UAVs only in simulation.

Three low-concurrency extraction agents grounded the papers, two disjoint workers drafted the source pages, and a separate read-only reviewer checked the completed batch. Review corrected the ADS expansion, narrowed scale reconfiguration to changing visible NTN devices, removed a false recurrent-policy description, fixed two MRLMN mechanism summaries and two author affiliations, clarified STAR-RIS sensor and SKYSCALE segment geometry, and added three reciprocal SLIPT concept links.

Validation reports **569 sources / 570 concepts / 150 entities / 612 raw folders** and **42 genuinely new sources** remaining. Wikilinks are clean; process narration is absent outside this log; the index covers all **1,341** catalogue-able pages exactly once; all **1,339** frontmatter blocks validate; entity rosters have zero overclaims (33 advisory omissions/namesakes remain); all 12 wiki-tool unit tests pass; and `git diff --check` is clean. The optional local LLM Wiki health endpoint is unreachable, so file-grounded CLI audits remain authoritative.
...
- [[lyu-2023-isac-maneuver-beamforming]] - Lyu et al. 2023, *IEEE TWC*, DOI `10.1109/TWC.2022.3211533`. Sensing-only SDPs and graph reachability identify feasible UAV locations before alternating SCA/SDR maneuver and beam design.
- [[fan-2026-directional-neighbor-discovery]] - Fan et al. 2026, *IEEE TMC*, DOI `10.1109/TMC.2025.3649859`. Synchronous and asynchronous directional discovery minimize power-delay surrogates through GP/CCP, with a four-node chamber experiment.
- [[xie-2026-uav-irs-eppo]] - Xie et al. 2026, *IEEE TMC*, DOI `10.1109/TMC.2025.3600682`. EPPO combines neural episodic state abstraction, mogrifier LSTM, and closed-form LoS IRS phases for urban UAV trajectory control.
- [[du-2026-hierarchical-coalition-deployment]] - Du et al. 2026, *IEEE TGCN*, DOI `10.1109/TGCN.2025.3610747`. Data-similarity-aware ground coalitions are nested inside partial-space adaptive play for discrete UAV placement.
- [[shah-2026-cellfree-mimo-fap-control]] - Shah et al. 2026, *IEEE TWC*, DOI `10.1109/TWC.2026.3685988`. A three-stage APG, GA, and DBSCAN/conflict-repair pipeline controls aerial cell-free FAP trajectories, powers, associations, and pilot reuse.

New vocabulary pages are [[sensing-feasible-uav-reachability]], [[directional-neighbor-discovery]], [[power-delay-product]], [[neural-episodic-control-with-state-abstraction]], [[mogrifier-lstm-policy]], [[closed-form-irs-phase-alignment]], [[data-similarity-aware-coalition-formation]], [[partial-space-adaptive-play]], [[aerial-terrestrial-cell-free-massive-mimo]], [[accelerated-proximal-gradient-trajectory-power-control]], and [[interference-aware-dbscan-pilot-assignment]]. Matching biographies and affiliations established [[guangxu-zhu]], [[zhe-song]], [[xuanhe-yang]], [[shuai-wang]], [[gaofeng-pan]], and [[chee-yen-leow]]. Existing Jie Xu, Dusit Niyato, Geng Sun, Jiahui Li, and Jiacheng Wang rosters were extended; the exact-name Haichao Wang recurrence remains unmerged because identity evidence is insufficient.

Metadata notes: the first, second, and fourth parses omit final publication metadata, so exact-title Crossref records supplied their DOI, venue, and year. The aerial-IRS parse records August 2025 early access, while its DOI record places the final article in TMC 25(1), January 2026; the source slug and citation use the final issue year. The cell-free parse prints its DOI but omits venue/year, which the DOI record verifies as TWC 25 (2026), pages 15818-15832.

Evidence caveats: the ISAC source models only transmit illumination and its reachability proof does not explicitly enforce the mission hop budget. The neighbor-discovery hardware is a static chamber emulation, while mobility and Poisson discovery remain model assumptions. The aerial-IRS study omits control/IRS energy and has reward, fairness, path-loss, and urban-layout inconsistencies. The hierarchical game proves inner unilateral stability and only a conditional outer equilibrium, not a global joint optimum. The cell-free comparisons change user/FAP/antenna configurations, its stated Table III percentage is arithmetically inconsistent, and the implemented DBSCAN is spatial clustering followed by interference-conflict repair rather than interference-aware clustering itself.

Four low-concurrency extraction workers covered the first four papers while the cell-free source was extracted locally. An independent cell-free reviewer corrected the DBSCAN mechanism, an absent separation constraint, Option 8 overhead wording, APG candidate selection, comparison configurations, and a percentage inconsistency. A second independent review corrected an unsupported aerial-IRS energy interval, the episodic reward formula, coalition-stage wording and stability scope, and an undisclosed ISAC location-grid assumption. The maintained `make_batches.py --batch 1` selector produced the explicit allowlist; no new recurring tooling need arose beyond the already tested toolkit.

Validation before commit: `corpus_counts.py` reported 504 sources, 476 concepts, 111 entities, 15 findings, 15 synthesis pages, 6 comparisons, 6 methodology pages, 5 queries, 3 thesis pages, 2 reference pages, and 612 raw folders. `curation_status.py --dupes` reported 505 path/title-matched folders and 107 genuinely new uncurated sources; its nonzero exit is expected while the backlog remains. `linkcheck.py --orphans`, `process_refs.py`, `index_audit.py`, `frontmatter_audit.py`, `entity_roster_audit.py`, all 12 toolkit tests, and `git diff --check` were clean after repairing formatter-stripped references during the aerial-IRS year/slug correction. Index coverage was 1143/1143, frontmatter coverage was 1141 pages, and the entity audit found 0 claimed-but-absent over-claims with 29 advisory present-but-unlisted matches. The optional LLM Wiki health endpoint returned HTTP `502`, so no live graph counts were available.
...
- [[zhang-2026-air-sea-isac-inspection]] - Zhang et al. 2026, *IEEE TWC*, DOI `10.1109/TWC.2026.3694754`. A UAV and USV jointly plan maritime inspection through target clustering, heterogeneous Bi-TSPN routing, SCA refinement, and alternating communication/sensing beamforming and current-aware motion.
- [[wang-2026-robust-anti-uav-isac]] - Wang et al. 2026, *IEEE TWC*, DOI `10.1109/TWC.2026.3707705`. A multi-UAV team assigns spatially separated sensing-transmitter and sensing-receiver roles, then jointly optimizes robust beamformers and trajectories against bounded target-position error.
- [[wang-2025-cellular-uav-cooperative-detection]] - Wang et al. 2025, *IEEE TWC*, DOI `10.1109/TWC.2024.3509978`. A ground BS and connected UAV estimate surrounding UAV states, associate targets by normalized position/motion distance, fuse them with an EKF, and update beamforming and trajectory.
- [[jing-2024-isac-trajectory-localization]] - Jing et al. 2024, *IEEE TWC*, DOI `10.1109/TWC.2024.3396571`. A multi-stage estimate-design-sense loop alternates UAV trajectory and bandwidth design with grid maximum-likelihood localization from accumulated ranges.
- [[lu-2026-icsn-beamforming]] - Lu et al. 2026, *IEEE TGCN*, DOI `10.1109/TGCN.2026.3709306`. Wide-beam acquisition supplies angular estimates and CRB confidence intervals, then fractional-programming beam updates narrow the communication, sensing, and navigation beams.

New concept pages are [[bi-traveling-salesman-problem-with-neighborhoods]], [[spatially-separated-uav-isac-role-scheduling]], [[ground-air-cooperative-isac-detection]], [[multi-stage-estimate-design-sense-trajectory]], [[integrated-communication-sensing-navigation]], and [[crb-guided-angular-confidence-beamforming]]. Existing ISAC, networked sensing, CRB, data-fusion, trajectory, propulsion, localization, fractional-programming, and AO/SDR/SCA pages were extended rather than duplicated.

Matching biographies, degree histories, affiliations, emails, and research profiles established [[qixun-zhang]] across three sources, [[kun-yang]] across four, and [[fan-liu]] and [[christos-masouros]] across three each. The networked-ISAC parse reverses Fan Liu's B.Eng./Ph.D. years while repeating his career history; the conflict is recorded without weakening the otherwise exact identity match. [[yong-zeng]] was reconciled to thirteen sources and his current Southeast University / Purple Mountain Laboratories role, while [[zhiyong-feng]] was extended to five sources and given reciprocal source links. The Zhaolong Ning byline in the anti-UAV paper remains unlinked because that parse supplies no biography, affiliation, or email with which to confirm the existing entity.

Metadata notes: exact-title Crossref records supplied or confirmed the final records used here. The air-sea and robust anti-UAV papers appear in 2026 TWC volume 25 at pages 17479-17493 and 19611-19627. The cooperative-detection paper appears in 2025 TWC 24(2), 1541-1554; the localization paper in 2024 TWC 23(10), 12857-12872; and the ICSN paper is a 2026 TGCN early-access record at pages 1-1. Technical claims, algorithms, assumptions, and numerical findings remain grounded in the complete local parses.

Evidence caveats: all five papers are simulation studies. The air-sea source is adjacent to UAV-USV MEC but does not optimize computation offloading, and its parse contains hover-duration, objective-label, missing-subproblem, and damaged-symbol conflicts. The robust anti-UAV source optimizes a first-order robust approximation and its prose/figure disagree on one uncertainty trend. The cooperative-detection source reports conflicting 67%/38% accuracy summaries and does not identify the denominator for its range/velocity or rate improvements. The localization source notes that its CRB can be loose for nonlinear range-to-coordinate estimation. The ICSN source models navigation as distribution of angular information rather than end-to-end navigation error and gives no energy metric despite its green-network venue.

Two independent read-only reviewers checked all five source pages, six concepts, author identities, reciprocal links, index, and overview against the complete extraction reports and targeted raw parses. Their corrections removed an unsupported BS/UAV array-size comparison; added five missing reciprocal concept backlinks; reconciled three earlier Yong Zeng sources and his current affiliation; documented Fan Liu's reversed degree-year typo; repaired two stale raw-artifact paths; and completed older reciprocal links for the touched Zhiyong Feng roster. They found no remaining high-confidence technical, numerical, metadata, identity, vocabulary, index, overview, or wikilink defect.

Validation before commit: all 9 maintained toolkit tests passed, and `python tools/wiki/process_refs.py` reported 0 affected files. `python tools/wiki/corpus_counts.py --json counts-batch28-final-2026-07-13.json` reported 489 sources, 454 concepts, 94 entities, 15 findings, 15 synthesis pages, 6 comparisons, 6 methodology pages, 5 queries, 3 thesis pages, 2 reference pages, and 612 raw-source folders. `python tools/wiki/curation_status.py --dupes --json status-batch28-final-2026-07-13.json` reported 490 path/title-matched curated folders and 122 genuinely new uncurated folders; its nonzero exit is expected while the backlog remains. `python tools/wiki/linkcheck.py --orphans`, `python tools/wiki/index_audit.py`, `python tools/wiki/frontmatter_audit.py`, `python tools/wiki/entity_roster_audit.py`, and `git diff --check` were clean; index coverage was 1089/1089 catalogue-able pages, frontmatter coverage was 1087 pages, and the entity audit reported 0 claimed-but-absent over-claims with 31 advisory present-but-unlisted omissions. The LLM Wiki health and graph endpoints returned HTTP `502`, so no live node/edge counts were available.

## [2026-07-13] Curated mountainous ILAC, semantic twinning, graph anti-jamming, pretrained cooperation, and crack evaluation

Added five source pages, six concept pages, and three author entities:

- [[xie-2026-geoagg-hsac]] - Xie et al. 2026, *IEEE TWC*, DOI `10.1109/TWC.2025.3625295`. GeoAgg-HSAC uses terrain-occlusion-aware graph contrastive pretraining and hybrid SAC to control UAV trajectories, powers, and user associations for mountainous integrated localization and communication.
- [[liao-2026-semantic-twinning-tracking]] - Liao et al. 2026, *IEEE TMC*, DOI `10.1109/TMC.2026.3700322`. Goal-oriented semantic twinning combines inferability-driven state sampling, multiple reconstruction models, MADDPG, EWC, and mixed replay for satellite-UAV collaborative target tracking.
- [[tang-2026-gat-antijamming]] - Tang et al. 2026, *IEEE TWC*, DOI `10.1109/TWC.2025.3618614`. A frozen GAT supplies interference-aware beamformers inside a two-agent MADDPG deployment and jammer-power game.
- [[chen-2026-hammurabi-cooperation]] - Chen et al. 2026, *IEEE TPDS*, DOI `10.1109/TPDS.2026.3654605`. Hammurabi diagnoses cooperation in rule-pretrained policies through social-dilemma tools, then fine-tunes parameter-shared policies with inequality-aversion shaping.
- [[cheng-2026-cnn-mamba-cracks]] - Cheng et al. 2026, *IEEE T-ITS*, DOI `10.1109/TITS.2026.3673474`. WTCMamba combines dual-convolution features, contextual skeleton/detail propagation, Haar-wavelet guidance, and selective state-space decoding; an LMC-Belloch/Triton scan is benchmarked on Jetson AGX Orin separately from the proposed air-ground acquisition platform.

New concept pages are [[terrain-occlusion-aware-graph-state-aggregation]], [[goal-oriented-semantic-twinning]], [[hierarchical-graph-anti-jamming-control]], [[pretrained-policy-cooperation-shaping]], [[parameter-sharing-marl]], and [[wavelet-guided-mamba-crack-segmentation]]. Existing localization/communication, terrain-channel, hybrid-action, graph-network, semantic-communication, AoI, digital-twin, MADDPG, anti-jamming, warm-start, fairness, edge-intelligence, and UAV-edge-inference pages were extended rather than duplicated. Biography, degree, affiliation, and recurring-source evidence established [[qi-qi]] across two sources and [[shaohua-wu]] and [[qinyu-zhang]] across three and four sources. [[zheng-chang]], [[zhu-han]], and [[dusit-niyato]] were reconciled to six, eighteen, and fifty-one sources; the last total includes six exact-name earlier source matches that had remained entity-audit advisories.

Metadata notes: the local parses vary in final publication detail. Exact-title Crossref DOI records supplied or confirmed the final records used here: TWC 25, 6507-6522 for GeoAgg-HSAC; TMC early access, 1-15 for semantic twinning; TWC 25, 5432-5445 for graph anti-jamming; TPDS 37(3), 744-761 for Hammurabi; and T-ITS 27(7), 7756-7773 for CNN-Mamba. The Hammurabi and CNN-Mamba pages explicitly distinguish those external publication records from technical claims grounded in the complete local parses.

Evidence caveats: the five studies are simulation-, dataset-, or processor-benchmark-led rather than full field deployments. GeoAgg-HSAC uses one reconstructed mountain environment and contains conflicting GDOP, reward, collision, link-index, and optimization expressions. Semantic twinning has AoI-sign, discrete-action, stage/time-axis, and CTDE ambiguities, and the aggregation behind its headline target-loss percentage is not reproducible from the parse. The anti-jamming method assumes perfect global CSI and fixed altitude, reports empirical convergence rather than a proven saddle point, and defines a hierarchy without distinct update periods. Hammurabi's finite-state Nash-Q theorem does not directly cover the continuous actor-critic implementations. WTCMamba uses public datasets and a Jetson scan benchmark; the proposed UAV/vehicle acquisition platform and heuristic road-risk scores are not quantitatively validated end to end.

Two independent read-only reviewers checked all five source pages, concepts, author identities, reciprocal links, index, and overview against the complete extraction reports and targeted raw parses. Ten correction classes were applied: defining semantic significance by inferability rather than task importance; recording GeoAgg's retained LoS/NLoS state concatenation; removing unsupported two-timescale language from the graph hierarchy; changing the target-loss issue from inconsistency to underspecification; adding seven reciprocal concept backlinks; stating Hammurabi's fine-tuning step; separating the LMC-Belloch scan from the proposed air-ground platform; reconciling six earlier Dusit Niyato sources; documenting Qi Qi's conflicting professor/associate-professor ranks; and marking externally verified final citation records.

Validation before commit: `python tools/wiki/tests/test_curation_status.py` passed all 6 tests, `python tools/wiki/tests/test_process_refs.py` passed all 3 tests, and `python tools/wiki/process_refs.py` reported 0 affected files. `python tools/wiki/corpus_counts.py --json counts-batch27-final-2026-07-13.json` reported 484 sources, 448 concepts, 90 entities, 15 findings, 15 synthesis pages, 6 comparisons, 6 methodology pages, 5 queries, 3 thesis pages, 2 reference pages, and 612 raw-source folders. `python tools/wiki/curation_status.py --dupes --json status-batch27-final-2026-07-13.json` reported 485 path/title-matched curated folders and 127 genuinely new uncurated folders; its nonzero exit is expected while the backlog remains. `python tools/wiki/linkcheck.py --orphans`, `python tools/wiki/index_audit.py`, `python tools/wiki/frontmatter_audit.py`, `python tools/wiki/entity_roster_audit.py`, and `git diff --check` were clean; index coverage was 1074/1074 catalogue-able pages, frontmatter coverage was 1072 pages, and the entity audit reported 0 claimed-but-absent over-claims with 33 advisory present-but-unlisted omissions. LLM Wiki health returned HTTP `200`; the graph response contained 1000 nodes and 9038 edges.

## [2026-07-13] Curated fluid-antenna relaying, full-space STAR-RIS security, terrain coverage, UAV crowdsensing, and anti-UAV deployment

Added five source pages, three concept pages, and four author entities:

- [[zhu-2026-fas-uav-fbl]] - Zhu et al. 2026, *IEEE TWC*, DOI `10.1109/TWC.2026.3688660`. A fluid-antenna receiver supplies effective spatial diversity for finite-blocklength DF UAV relaying, while port probing, switching, blocklength, altitude, and relay power determine reliability and energy efficiency.
- [[meng-2026-fullspace-star-ris-secure]] - Meng and Wu 2026, *IEEE TGCN*, DOI `10.1109/TGCN.2025.3581390`. DS-JO lets a UAV cross the STAR-RIS plane by coupling side-dependent reflection/transmission roles, robust secrecy coefficients, and trajectory SCA under colluding-eavesdropper CSI uncertainty.
- [[zhou-2026-gl-ahg-coverage-planning]] - Zhou et al. 2026, *IEEE TMC*, DOI `10.1109/TMC.2026.3682715`. GL-AHG weights terrain waypoints by a distance/angle energy proxy, applies game-learning weighted vertex cover, and alternates distance- and energy-heavy genetic route search.
- [[guang-2026-hiswta-mcs]] - Guang et al. 2026, *IEEE TMC*, DOI `10.1109/TMC.2025.3621440`. HISWTA dynamically clusters sensing UAVs, routes fused information among heads, self-heals weak heads, and allocates tasks with approximate Shapley values.
- [[ma-2026-game-ibs-deployment]] - Ma et al. 2026, *IEEE TWC*, DOI `10.1109/TWC.2026.3695955`. Multiple fixed interference base stations disrupt an unauthorized UAV's GNSS and operator link while constraining friendly-device leakage; exhaustive placement uses a SAC UAV path response for each candidate.

New concept pages are [[fluid-antenna-system]], [[full-space-star-ris-uav-trajectory]], and [[anti-uav-interference-base-station-deployment]]. Existing finite-blocklength, STAR-RIS, physical-layer-security, visual-coverage, genetic-algorithm, UAV-clustering, swarm-hierarchy, first-order-radio-energy, energy-balancing, mobile-crowdsensing, and potential-game pages were extended rather than duplicated. Biography, affiliation, degree, and recurring-source evidence established [[kai-kit-wong]] across six curated sources, [[chan-byoung-chae]] across three, and [[riheng-jia]] and [[minglu-li]] across two each.

Metadata notes: the local parses do not all print final publication records. Exact-title Crossref matches supplied or confirmed the final DOI, venue, year, volume, issue, and pages used above: TWC 25, 16714-16729 for the FAS paper; TGCN 10, 376-388 for the STAR-RIS paper; TMC 1-15 for GL-AHG; TMC 25(3), 3728-3743 for HISWTA; and TWC 25, 17724-17739 for the IBS paper. Technical claims, models, algorithms, limitations, and numerical findings remain grounded in the complete local Markdown parses.

Evidence caveats: all five sources are analytical or simulation studies rather than field deployments. The FAS derivation validates an eigenvalue-weighted surrogate rather than the original correlated physical-port channel and has UE/UAV energy-label ambiguity. The STAR-RIS design is local/approximate, uses fixed altitude and two users, and compares some trajectories with different endpoints. GL-AHG uses a simplified linear energy proxy and reports no repeated-run statistics, runtime scaling, or flight test. HISWTA omits propulsion despite mobile UAVs and contains routing, fuzzy-rule, priority, and Shapley-definition inconsistencies. The IBS source has exponential exhaustive placement and unresolved finite-versus-continuous strategy, potential-sign, equilibrium/global-optimum, motion-dimension, and grid-count conflicts.

Two independent read-only reviewers checked the five source pages, concepts, author identities, reciprocal links, index, and overview against the complete extraction reports and targeted raw parses. Four mandatory corrections were applied: removing an unsupported heterogeneous-fleet link from the single-UAV GL-AHG paper; restoring the exact `Ground–Air` title punctuation; limiting finite-improvement and best-response guarantees to finite-strategy potential games; and broadening mobile crowdsensing to include UAV-only sensing swarms. The anti-UAV index entry now presents the exact-potential classification as the paper's claim rather than a verified theorem.

Validation before commit: `python tools/wiki/tests/test_curation_status.py` passed all 6 tests, `python tools/wiki/tests/test_process_refs.py` passed all 3 tests, and `python tools/wiki/process_refs.py` reported 0 affected files. `python tools/wiki/corpus_counts.py --json counts-batch26-final-2026-07-13.json` reported 479 sources, 442 concepts, 87 entities, 15 findings, 15 synthesis pages, 6 comparisons, 6 methodology pages, 5 queries, 3 thesis pages, 2 reference pages, and 612 raw-source folders. `python tools/wiki/curation_status.py --dupes --json status-batch26-final-2026-07-13.json` reported 480 path/title-matched curated folders and 132 genuinely new uncurated folders; its nonzero exit is expected while the backlog remains. `python tools/wiki/linkcheck.py --orphans`, `python tools/wiki/index_audit.py`, `python tools/wiki/frontmatter_audit.py`, `python tools/wiki/entity_roster_audit.py`, and `git diff --check` were clean; index coverage was 1060/1060 catalogue-able pages, frontmatter coverage was 1058 pages, and the entity audit reported 0 claimed-but-absent over-claims with 39 advisory present-but-unlisted omissions. LLM Wiki health and graph endpoints returned HTTP `200`; the graph response contained 1000 nodes and 9187 edges.

## [2026-07-13] Curated UAV-HSR jitter, FALCON semantic links, fast aerial deployment, federated bandits, and wind-aware relaying

Added five source pages, three concept pages, and four author entities:

- [[liu-2026-uav-hsr-jitter]] - Liu et al. 2026, *IEEE T-ITS*, DOI `10.1109/TITS.2026.3677161`. Gaussian-random-walk UAV jitter drives CA/DA outage and high-SNR rate analysis; a nearest-codebook beamwidth rule balances directivity, accumulated misalignment, and scan time.
- [[niu-2026-falcon-semantic]] - Niu et al. 2026, *IEEE TMC*, DOI `10.1109/TMC.2026.3709169`. FALCON combines KANet/shared-prompt multimodal alignment, channel-aware Sparsemax token selection, and range-null DDIM-style signal recovery.
- [[zhang-2019-fast-uav-deployment]] - Zhang and Duan 2019, *IEEE TMC*, DOI `10.1109/TMC.2018.2840143`. Min-max and min-sum travel delay produce exact, FPTAS, bounded-greedy, and pseudo-polynomial algorithms for heterogeneous UAV coverage deployment.
- [[qian-2026-federated-bandit-aircomp]] - Qian et al. 2026, *IEEE TMC*, DOI `10.1109/TMC.2026.3651589`. Event-triggered federated LinUCB sends Gram/reward statistics through AirComp while BCD-ADMM controls client power, receive normalization, and UAV trajectory.
- [[zhu-2026-fixed-wing-fd-af-wind]] - Zhu et al. 2026, *IEEE TGCN*, DOI `10.1109/TGCN.2025.3603164`. Wind-triangle case analysis minimizes propulsion energy for a fixed-wing full-duplex AF relay and derives air speed, flight time, crab angle, and pitch angle.

New concept pages: [[jitter-aware-uav-beamwidth-control]] distinguishes statistical platform-jitter beamwidth selection from flight-state beam tracking and FANET breakage prediction; [[fast-heterogeneous-uav-deployment]] separates travel-to-coverage delay from static placement and disk-count objectives; and [[federated-linear-bandit-learning]] separates online regret/statistic aggregation from gradient-based federated learning. FALCON and the wind-aware relay extend the established [[multi-modal-semantic-communication]], [[generative-diffusion-model]], [[over-the-air-computation]], and [[fixed-wing-propulsion-energy-model]] vocabulary rather than creating source-specific near-synonyms.

Biography, affiliation, degree, and research-profile evidence established [[george-k-karagiannidis]], [[yuanming-shi]], and [[xingwang-li]] across three curated sources each. Matching Southern University of Science and Technology biographies established [[weijie-yuan]] across FALCON, cargo pickup, and digital-twin deployment. His byline in the covert-transmission source lacks a local biography, affiliation, or email, so that fourth occurrence remains an entity-audit advisory rather than an asserted identity edge.

Metadata notes: the UAV-HSR parse supplies the journal and DOI but not the final volume/issue/pages; its DOI record supplied the 2026 T-ITS issue details. The FALCON, fast-deployment, and wind-relay parses omit complete final records, so exact-title Crossref matches supplied their final TMC/TGCN years, venues, pages, and DOIs. The federated-bandit parse supplies its 2026 dates and journal DOI while Crossref confirms the final TMC volume, issue, and pages. The GLOBECOM 2023 DOI in that parse is an earlier conference version, not the journal DOI. Technical claims, assumptions, algorithms, and numerical findings remain grounded in the local Markdown parses.

Evidence caveats: all five are analytical, simulation-, workstation-, or dataset-based rather than full airborne communication deployments. The jitter paper uses rural free-space/LoS and high-SNR/DA approximations, does not validate its Gaussian disturbance model against flight traces, and contains outage-direction and curvature wording conflicts. FALCON uses workstation inference under AWGN/Rayleigh channels, adapts a unimodal baseline, has no isolated channel-estimation sensitivity test, and treats embedded feasibility as theoretical. Fast deployment reduces the core geometry to one dimension, imposes order preservation, omits interference, and loses accuracy under radius heterogeneity. The federated-bandit design assumes accurate CSI, tight synchronization, error-free downlink, one shared fixed parameter, fixed altitude, and no propulsion energy; its centralized trajectory solve may not scale. The wind-relay paper assumes constant wind, a fixed straight level route, free-space channels, perfect Doppler compensation, and propulsion-only energy for one source-relay-destination topology.

Two independent read-only reviewers checked the source pages, concepts, author identities, reciprocal links, index, and overview against the complete extraction reports and targeted raw parses. Six unique mandatory correction classes were applied: removing an ordinary-JSCC to distributed-video-JSCC edge; replacing an invented `Q` bit unit with an unspecified data amount; keeping the federated-bandit definition broad enough for heterogeneous models; dropping the identity-unsupported Weijie Yuan covert-source edge; updating modified source dates; and removing run-oriented wording from the evergreen overview. The reviewers found no remaining high-confidence technical, metadata, identity, link, count, or concept-boundary defect.

Validation results before commit: `python tools/wiki/tests/test_curation_status.py` passed all 6 tests, `python tools/wiki/tests/test_process_refs.py` passed all 3 tests, and `python tools/wiki/process_refs.py` reported 0 affected files. `python tools/wiki/corpus_counts.py --json counts-batch25-final-2026-07-13.json` reported 474 sources, 439 concepts, 83 entities, 15 findings, 15 synthesis pages, 6 comparisons, 6 methodology pages, 5 queries, 3 thesis pages, 2 reference pages, and 612 raw-source folders. `python tools/wiki/curation_status.py --dupes --json status-batch25-final-2026-07-13.json` reported 475 path/title-matched curated folders and 137 genuinely new uncurated folders; its nonzero exit is expected while the backlog remains. `python tools/wiki/linkcheck.py --orphans`, `python tools/wiki/index_audit.py`, `python tools/wiki/frontmatter_audit.py`, `python tools/wiki/entity_roster_audit.py`, and `git diff --check` were clean; index coverage was 1048/1048 catalogue-able pages, frontmatter coverage was 1046 pages, and the entity audit reported 0 claimed-but-absent over-claims with 39 advisory present-but-unlisted omissions. LLM Wiki health and graph endpoints returned HTTP `200`; the current graph response contained 1000 nodes and 9260 edges.

## [2026-07-13] Curated dual-domain RIS harvesting, marine NOMA computation, radio-map cargo pickup, resilient A2G coverage, and secure mobile IoV

Added five source pages, four concept pages, and two author entities:

- [[peng-2023-dual-domain-eh-ris]] - Peng and Wang 2023, *IEEE TWC*, DOI `10.1109/TWC.2023.3245820`. A UAV-mounted RIS combines harvest/transmit time splitting with element-level reflection/harvesting, while SD3 controls harvesting duration, power, element scheduling, and phases under QoS constraints.
- [[wang-2026-noma-marine-data-computation]] - Wang et al. 2026, *IEEE TGCN*, DOI `10.1109/TGCN.2025.3605919`. Marine sensing devices upload required data through NOMA for UAV computation; min-max-normalized TD3 jointly controls movement, device powers, and aerial compute allocation.
- [[cao-2026-radio-map-cargo-pickup]] - Cao et al. 2026, *IEEE T-ITS*, DOI `10.1109/TITS.2025.3639674`. UPSEEO thresholds an expected-SNR map, computes all-pairs communication-feasible A* paths, and uses PSO trip allocation with payload-dependent speed selection.
- [[wang-2026-robust-multiuav-jtcra]] - Wang et al. 2026, *IEEE TWC*, DOI `10.1109/TWC.2026.3657894`. Parameter-shared MAPPO and QMIX controllers coordinate trajectories, power, and bandwidth so surviving UAVs cover users after energy-driven platform dropouts while meeting a Jain-fairness requirement.
- [[li-2026-secrecy-ee-uav-ris-iov]] - Li et al. 2026, *IEEE TWC*, DOI `10.1109/TWC.2025.3594691`. Vehicle jamming and a UAV-mounted RIS protect traffic from an untrusted forwarding relay; Dinkelbach/CCCP, MM, and firefly-warm-started DDPG optimize secrecy energy efficiency.

New concept pages: [[softmax-deep-double-deterministic-policy-gradients]] distinguishes SD3's softmax target from TD3's strict twin-critic minimum; [[dual-domain-ris-energy-harvesting]] records joint time splitting and RIS-element partitioning; [[radio-map-aided-uav-path-planning]] separates radio-map route feasibility from radio-map channel estimation; and [[secrecy-energy-efficiency]] makes the metric's paper-specific energy-accounting boundary explicit. Biography, affiliation, and recurring-source evidence established [[qian-wang]] across three marine-IoT papers and [[fuhui-zhou]] across four wireless-powered MEC, UAV optimization, tracking, and security papers. [[liping-qian]], [[bin-lin]], [[yuan-wu]], and [[victor-c-m-leung]] were reconciled to seven, twelve, nineteen, and eleven sources. Recurring Jiangling Cao, Dingcheng Yang, Tiankui Zhang, Xianbin Wang, and Hongbo Zhao names were not promoted without stronger cross-parse identity evidence.

Metadata notes: the RIS parse omits its article record, so an exact-title Crossref match supplied the 2023 TWC record. The marine-data and secure-IoV parses expose 2025 online publication dates but not their final issue records; exact-title Crossref matches place both in 2026 issues, so their source slugs and citation years use 2026 rather than the early-access year. Exact-title Crossref also supplied the parse-silent 2026 T-ITS cargo record and 2026 TWC A2G-robustness record. The GLOBECOM 2024 DOI inside the marine parse is a conference precursor, not the journal DOI. Technical claims, algorithms, assumptions, and numerical findings remain grounded in the local Markdown parses.

Evidence caveats: all five papers are simulation-only. The dual-domain RIS study measures harvested-energy efficiency rather than battery lifetime, omits propulsion, fixes altitude, assumes ideal phases/interference cancellation, and has conflicting contribution-versus-table percentages plus a stray convex/non-convex wording conflict. The marine controller uses one fixed-altitude UAV, fixed sensing devices, negligible feedback cost, non-released compute allocations, and a singular-at-zero simplified propulsion term; its displayed constraints omit the prose return-to-start condition. The cargo planner assumes an accurate static expected-SNR map, ignores handover delay and communication energy, uses 2-D fixed-altitude paths, and relies on heuristic PSO. The resilient A2G design assumes modeled threshold dropouts and satellite user reports, excludes recharge-and-return service, derives association rather than learning it, and contains ambiguous communication-agent counting and damaged allocation equations. The secure-IoV design excludes UAV energy under continuous solar supply, gives a sub-optimal trajectory, leaves closed-loop Doppler compensation for future work, and contains per-slot/trajectory-objective, coordinate, and MM/CCCP terminology conflicts.

Two independent read-only reviewers checked the five source pages, four concept pages, two entity pages, reciprocal edits, index, and overview against the complete parses and extraction reports. Three high-confidence corrections were applied: Qian Wang is now attributed only to sources she actually co-authored; the radio-map concept no longer infers changing-obstacle adaptation beyond the parse; and the secure UAV-MEC comparison now includes UAV-side computation in its secrecy-efficiency denominator. The reviewers found no further high-confidence technical, metadata, identity, link, count, or process-narration defect.

The audit also found a pre-existing `this pass` phrase in [[kezhi-wang]] that the maintained process scanner missed. The page was rewritten as evergreen reference prose, `process_refs.py` now detects standalone `this pass`, `tools/wiki/README.md` documents the rule, and `test_process_refs.py` adds three regression tests while preserving ordinary-passage and mini-batch exemptions.

Validation results before commit: `python tools/wiki/tests/test_curation_status.py` passed all 6 tests, `python tools/wiki/tests/test_process_refs.py` passed all 3 tests, and `python tools/wiki/process_refs.py` reported 0 affected files. `python tools/wiki/corpus_counts.py --json counts-batch24-final-2026-07-13.json` reported 469 sources, 436 concepts, 79 entities, 15 findings, 15 synthesis pages, 6 comparisons, 6 methodology pages, 5 queries, 3 thesis pages, 2 reference pages, and 612 raw-source folders. `python tools/wiki/curation_status.py --dupes --json status-batch24-final-2026-07-13.json` reported 470 path/title-matched curated folders and 142 genuinely new uncurated folders; its nonzero exit is expected while the backlog remains. `python tools/wiki/linkcheck.py --orphans`, `python tools/wiki/index_audit.py`, `python tools/wiki/frontmatter_audit.py`, `python tools/wiki/entity_roster_audit.py`, and `git diff --check` were clean; index coverage was 1036/1036 catalogue-able pages, frontmatter coverage was 1034 pages, and the entity audit reported 0 claimed-but-absent over-claims with 38 advisory present-but-unlisted omissions. LLM Wiki health and graph endpoints returned HTTP `502`, so no live node/edge counts were available.

## [2026-07-13] Curated solar-charging trajectories, BD-RIS/RSMA, UAV-FL grouping, aerial-IRS RSMA, and wireless-powered short packets

Added five source pages, two concept pages, and one author entity:

- [[zhang-2022-solar-charging-uav-iot]] - Zhang et al. 2022, *IEEE TMC*, DOI `10.1109/TMC.2021.3075083`. Action-confined Q-learning and SARSA route one solar-powered UAV among charging stations and serving points under data, net-energy, battery-depletion, and Jain-fairness terms.
- [[huroon-2026-bd-ris-rsma-uav]] - Huroon et al. 2026, *IEEE TWC*, DOI `10.1109/TWC.2025.3617169`. A quadratic transform and augmented GBD split group-connected BD-RIS cluster assignment from BCD/SCA/RCG optimization of RSMA precoders, common rates, UAV motion, and scattering matrices.
- [[fu-2026-uav-fl-user-grouping]] - Fu et al. 2026, *IEEE TGCN*, DOI `10.1109/TGCN.2026.3676914`. DBSCAN client grouping and two-phase UG-SCA jointly control participation, local data, UE power, hover time, and UAV trajectory under an expected-global-loss bound.
- [[mihertie-2026-aerial-irs-rsma-ee]] - Mihertie et al. 2026, *IEEE TGCN*, DOI `10.1109/TGCN.2026.3653184`. BCD/SCA and sequential rank-one relaxation optimize one UAV-mounted passive IRS placement, phases, RSMA precoders, and common rates under aggregate hardware distortion.
- [[xie-2023-wireless-powered-short-packet-uav]] - Xie et al. 2023, *IEEE TGCN*, DOI `10.1109/TGCN.2022.3218314`. A static UAV hybrid access point powers IoT devices before finite-blocklength TDMA upload; alternating SCA/fractional updates solve a continuous resource-allocation relaxation before heuristic integer-symbol rounding.

New concept pages: [[beyond-diagonal-ris]] distinguishes group-connected non-diagonal scattering from conventional diagonal IRS phases, and [[rate-splitting-multiple-access]] records the one-layer common/private stream structure used by both RSMA sources. The solar-charging, UAV-FL, and WPT short-packet sources reuse established trajectory, charging, harvesting, fairness, propulsion, FL, WPT, finite-blocklength, fractional-programming, and SCA vocabulary rather than creating source-specific near-synonyms. Matching degree years, institutions, Finnish research history, and research profile establish [[zheng-chang]] across five sources. No entity was created for the other recurring two-source bylines; Elhadj Moustapha Diallo's matching identity evidence retains conflicting current-employer text across parses.

Metadata notes: all five local parses expose titles and ordered bylines but omit their complete final publication records. Exact-title Crossref matches supplied or confirmed the final journal, year, DOI, volume/issue where available, and page ranges. The aerial-IRS parse misspells `Efficiency`; the exact-title record supplies the corrected published title. Technical claims, algorithms, assumptions, and numerical findings remain grounded in the local Markdown parses.

Evidence caveats: all five studies are simulation or theory based. The solar-charging controller uses one UAV, fixed candidate destinations, specified activity/solar models, and damaged battery/RL equations. The BD-RIS study assumes perfect CSI/synchronization and fixed users/RIS, demonstrates detailed mobility mainly for UAV 1, and contains global/local, manifold, caption, complexity-envelope, and editorial-residue conflicts. The UAV-FL study constrains a theoretical expected-loss bound under strong-convexity assumptions, omits aggregation energy, and has damaged feasibility/EGL fields. The aerial-IRS design excludes propulsion and battery energy, assumes perfect CSI and continuous lossless phases, and has several prose/plot conflicts whose values were not promoted. The WPT short-packet design uses one static hover point, linear harvesting, fixed decoding error, and a heuristic integer conversion, while omitting propulsion, circuitry, battery dynamics, and nonlinear rectifier behavior.

Two independent read-only reviewers checked the five source pages, two concept pages, Zheng Chang identity evidence, reciprocal links, index, and overview against the complete parses. Six unique corrections were applied: the FL no-group cutoff is now a setup-specific result rather than a theoretical bound; fixed-node wording was narrowed to the RIS and ground users; the solar/activity limitation no longer implies model knowledge; Crossref verification provenance was moved from evergreen citations to this log; generic RSMA wording was scoped to the one-layer form used here; and the short-packet TL;DR/index now distinguish continuous relaxation from heuristic rounding. The reviewers found no further high-confidence technical, identity, link, count, or metadata defect.

Validation results before commit: `python tools/wiki/tests/test_curation_status.py` passed all 6 tests, and `python tools/wiki/process_refs.py` reported 0 affected files. `python tools/wiki/corpus_counts.py --json counts-batch23-final-2026-07-13.json` reported 464 sources, 432 concepts, 77 entities, 15 findings, 15 synthesis pages, 6 comparisons, 6 methodology pages, 5 queries, 3 thesis pages, 2 reference pages, and 612 raw-source folders. `python tools/wiki/curation_status.py --dupes --json status-batch23-final-2026-07-13.json` reported 465 path/title-matched curated folders and 147 genuinely new uncurated folders; its nonzero exit is expected while the backlog remains. `python tools/wiki/linkcheck.py --orphans`, `python tools/wiki/index_audit.py`, `python tools/wiki/frontmatter_audit.py`, `python tools/wiki/entity_roster_audit.py`, and `git diff --check` were clean; index coverage was 1025/1025 catalogue-able pages, frontmatter coverage was 1023 pages, and the entity audit reported 0 claimed-but-absent over-claims with 40 advisory present-but-unlisted omissions. LLM Wiki health and graph endpoints returned HTTP `502`, so no live node/edge counts were available. No recurring curation failure or reusable tool gap appeared, so the stable toolkit was left unchanged.

## [2026-07-13] Curated UAV-assisted routing, embodied anti-jamming, cooperative relaying, emergency localization, and memory-augmented navigation

Added five source pages, five concept pages, and one author entity:

- [[fatemidokht-2021-vru-vanet-routing]] - Fatemidokht et al. 2021, *IEEE T-ITS*, DOI `10.1109/TITS.2020.3041746`. VRU_vu uses UAV-estimated road connectivity and trust filtering for urban VANET routing, while VRU_u applies ant-colony optimization as an aerial fallback when the vehicle graph is sparse.
- [[yang-2026-embodied-antijamming-uav]] - Yang et al. 2026, *IEEE TWC*, DOI `10.1109/TWC.2026.3677272`. Decentralized DDQN agents use prioritized replay and transferred source experience for U2U sub-band/power control under swept jamming while protecting U2I reuse.
- [[li-2016-energy-balanced-uav-relaying]] - Li et al. 2016, *IEEE TMC*, DOI `10.1109/TMC.2015.2467381`. An exact binary-integer min-max scheduler and the EPLA heuristic balance decoded-packet assignments, modulation, and derived forwarding power across cooperative UAV relays.
- [[tian-2026-joint-localization-communication]] - Tian et al. 2026, *IEEE TWC*, DOI `10.1109/TWC.2026.3656750`. AOA/ESPRIT distress localization feeds CoMP beamforming and SYNCORE DDQN control of UAV movement, time allocation, and transmit powers in an air-ground emergency system.
- [[he-2026-memdrl-uav-navigation]] - He et al. 2026, *IEEE TMC*, DOI `10.1109/TMC.2025.3612221`. MEMDRL combines MATD3, BeBold-style exploration, ConvLSTM observation histories, and summed multi-agent replay priorities for cooperative sensing and data-upload navigation.

New concept pages: [[uav-assisted-vanet-routing]], [[embodied-anti-jamming-resource-allocation]], [[energy-balanced-cooperative-uav-relaying]], [[joint-localization-and-communication]], and [[memory-augmented-multi-uav-navigation]]. Reciprocal links were added across the established ACO, FANET/ITS, anti-jamming, DDQN/PER, cooperative-relaying, energy-balancing, emergency-network, CoMP, trajectory, MA-POMDP, MATD3/CTDE, fairness, and data-collection pages. Matching biography, affiliation, and rare-name evidence established [[zehui-xiong]] across eleven generative-AI, semantic-communication, security, and network-optimization sources. The Tian byline extended [[zhu-han]] to fifteen sources.

Metadata notes: the local parses expose titles and bylines but omit or incompletely expose final publication records. Exact-title Crossref matches supplied or confirmed the five final journal records: Fatemidokht 2021 in T-ITS, Yang 2026 and Tian 2026 in TWC, and Li 2016 plus He 2026 in TMC. Technical claims, algorithms, assumptions, and numerical findings remain grounded in the local Markdown parses.

Evidence caveats: the routing study assumes trusted UAVs, fixed low-altitude urban operation, and negligible UAV energy, and it does not specify the trust threshold; its overhead equation and one result table contain inconsistencies. The anti-jamming study's evaluated embodiment is channel/jammer state rather than visual sensing; its jammer count, state description, baselines, and one plotted/prose result conflict, while the Raspberry Pi/USRP experiment lacks a complete field-deployment description. The relaying study is centralized and simulation-only, excludes propulsion energy, defines lifetime as time until all relay batteries are exhausted rather than first-relay failure, and contains percentage-denominator, equation-sign, and figure-axis issues. The emergency study uses one collaborative UAV, one rescuer, idealized LoS/Gaussian angle-error assumptions, and a simulation-only DDQN evaluation; its headline 43% baseline is unspecified and several equations are damaged in the parse. The navigation study uses random PoI data on two real-map layouts, omits radio energy and low-level flight control, evaluates at most ten UAVs, and relies on strong convergence assumptions whose appendix is absent from the parse.

Two independent read-only reviewers checked the five source/concept pairs, reciprocal links, author identities, index, and overview against the complete parses. They identified seven issues, all corrected: the relaying paper's nonstandard lifetime definition; first-hop UAV reports versus base-station second-hop SNR measurement; removal of an unsupported MINLP relation; the four-author VRU index byline; removal of obstacle memory from MEMDRL's observation claim; removal of the He and Tian communication/navigation anchors from direct computing/MEC track counts; and replacement of an unsupported successor claim with a thematic relation. The reviewers found no further high-confidence defect in the Yang source/concept pair, Zehui Xiong's eleven-source roster, Zhu Han's fifteen-source roster, or the exact-title metadata.

Validation results before commit: `python tools/wiki/tests/test_curation_status.py` passed all 6 tests, and `python tools/wiki/process_refs.py` reported 0 affected files. `python tools/wiki/corpus_counts.py --json counts-batch22-final-2026-07-13.json` reported 459 sources, 430 concepts, 76 entities, 15 findings, 15 synthesis pages, 6 comparisons, 6 methodology pages, 5 queries, 3 thesis pages, 2 reference pages, and 612 raw-source folders. `python tools/wiki/curation_status.py --dupes --json status-batch22-final-2026-07-13.json` reported 460 path/title-matched curated folders and 152 genuinely new uncurated folders; its nonzero exit is expected while the backlog remains. `python tools/wiki/linkcheck.py --orphans`, `python tools/wiki/index_audit.py`, `python tools/wiki/frontmatter_audit.py`, `python tools/wiki/entity_roster_audit.py`, and `git diff --check` were clean; index coverage was 1017/1017 catalogue-able pages, frontmatter coverage was 1015 pages, and the entity audit reported 0 claimed-but-absent over-claims with 40 advisory present-but-unlisted omissions. LLM Wiki health and graph endpoints returned HTTP `502`, so no live node/edge counts were available. No recurring curation failure or reusable tool gap appeared, so the stable toolkit was left unchanged.

## [2026-07-13] Curated hierarchical swarms, differentiated UAV services, emergency VNF control, segmented-IRS computing, and vehicular NOMA

Added five source pages, five concept pages, and one author entity:

- [[jia-2026-hierarchical-uav-swarms]] - Jia et al. 2026, *IEEE TWC*, DOI `10.1109/TWC.2025.3603432`. Head/tail UAV swarms use K-means/Voronoi predeployment and INS-WOA Pareto optimization for two-hop collection routes, user/relay powers, UAV energy, user energy, and delay.
- [[wang-2023-differentiated-uav-services]] - Wang et al. 2023, *IEEE TMC*, DOI `10.1109/TMC.2021.3116236`. Competing UAV owners imitate full-information experts, predict opponent actions, and adjust service quantities and fleet sizes under substitutable user demand.
- [[pham-2026-vnf-control-loop]] - Pham and Nguyen 2026, *IEEE TMC*, DOI `10.1109/TMC.2026.3682014`. Slot-level MADDPG multipath routing feeds an event-triggered BSUM VNF replication and placement controller for mission-critical emergency SFCs.
- [[hu-2026-segmented-irs-cpn]] - Hu et al. 2026, *IEEE TMC*, DOI `10.1109/TMC.2026.3678734`. MAPPO trajectories are coupled to phase alignment, user association, UAV compute allocation, and dynamically segmented IRS rows under a task-delay/energy objective.
- [[ahmed-2026-noma-irs-vehicular]] - Ahmed et al. 2026, *IEEE T-ITS*, DOI `10.1109/TITS.2025.3549224`. Projected fixed-point passive-IRS phases alternate with convex NOMA power allocation to improve UAV-to-vehicle sum capacity.

New concept pages: [[hierarchical-uav-swarm]], [[multi-agent-imitation-learning]], [[differentiated-uav-service-market]], [[routing-vnf-scaling-control-loop]], and [[fixed-point-irs-passive-beamforming]]. The segmented-IRS paper reuses [[dynamic-irs-user-association]] rather than creating a near-synonym. Reciprocal links were added to the established UAV-swarm, data-collection, multi-objective/WOA, game, GAN, NFV/SFC, MADDPG/BSUM, post-disaster, Computing Power Network, MAPPO, IRS/NOMA, and alternating-optimization pages. Matching biography, CQUPT affiliation, email, and recurring-coauthor evidence established [[zhaolong-ning]] across nine sources. The NUAA/Houston biographies extended [[ziye-jia]], [[qihui-wu]], and [[zhu-han]]; review also confirmed Zhu Han's previously unlisted semantic-successive-refinement source, bringing that roster to fourteen.

Metadata notes: the local parses expose titles and bylines but vary in publication detail. The hierarchical-swarm parse contains its DOI and 2025 early-access dates; the exact-title Crossref record supplies its final 2026 TWC volume/pages. The other four final journal records were absent or incomplete in their parses and were verified through exact-title Crossref matches: Wang 2023 TMC 22(4), Pham 2026 TMC early access, Hu 2026 TMC early access, and Ahmed 2026 T-ITS 27(2). Technical claims, algorithms, assumptions, and numerical findings remain grounded in the local Markdown parses.

Evidence caveats: all five papers are simulation or trace-driven studies rather than field deployments. The hierarchical-swarm optimizer freezes deployment/association before route/power optimization, omits explicit obstacle/collision constraints, and contains delay-cap and equation inconsistencies. The service-market study retains a globally informed operator despite decentralized owner policies, omits 3-D propagation/trajectory control, and has CNN/MLP, utility-normalization, and profit-label conflicts. The VNF study uses synthetic traffic with inconsistent node/request tables and a non-equivalent ideal baseline. The segmented-IRS study assumes fixed-agent training, full offloading, continuous phases, known geometry, and LoS-dominant channels, with a table/result mismatch on IRS size. The vehicular-NOMA study assumes one fixed-altitude UAV, perfect CSI/SIC, continuous phases, and no mobility process; its inner phase, SINR-reference, and outer alternating iterations have distinct or unstated stopping rules.

Two independent read-only reviewers checked the source/concept groups, entity evidence, reciprocal links, index, and overview against the parses. They identified seven issues, all corrected: globally informed operator scope; Zhu Han's fourteenth source; NUAA-versus-selected-Southeast affiliation wording for Ziye Jia; the video trace's parameterization role; removal of an inferred hierarchy benefit; separation of relay-backed and UAV-hosted Computing Power Networks; and separation of passive-IRS inner-loop stopping from joint-algorithm stabilization.

Validation results before commit: `python tools/wiki/tests/test_curation_status.py` passed all 6 tests, and `python tools/wiki/process_refs.py` reported 0 affected files. `python tools/wiki/corpus_counts.py --json counts-batch21-final-2026-07-13.json` reported 454 sources, 425 concepts, 75 entities, 15 findings, 15 synthesis pages, 6 comparisons, 6 methodology pages, 5 queries, 3 thesis pages, 2 reference pages, and 612 raw-source folders. `python tools/wiki/curation_status.py --dupes --json status-batch21-final-2026-07-13.json` reported 455 path/title-matched curated folders and 157 genuinely new uncurated folders; its nonzero exit is expected while the backlog remains. `python tools/wiki/linkcheck.py --orphans`, `python tools/wiki/index_audit.py`, `python tools/wiki/frontmatter_audit.py`, `python tools/wiki/entity_roster_audit.py`, and `git diff --check` were clean; index coverage was 1006/1006 catalogue-able pages, frontmatter coverage was 1004 pages, and the entity audit reported 0 claimed-but-absent over-claims with 40 advisory present-but-unlisted omissions. LLM Wiki health and graph endpoints returned HTTP `502`, so no live node/edge counts were available. No recurring curation failure or reusable tool gap appeared, so the stable toolkit was left unchanged.
