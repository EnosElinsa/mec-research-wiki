...
Crucially distinct from the wiki's [[friendly-jamming-uav]] concept: there, jamming is used **defensively** (a cooperative UAV jams an eavesdropper). Here jamming is the **threat** to be detected and evaded. In the wiki, [[shao-2024-drl-antijamming-mec]] is the anchor — it senses jammed subchannels ([[spectrum-sensing-channel-selection]]) and uses [[multi-agent-td3|PER-MATD3]] to jointly tune CPU frequency, bandwidth, and channel selection under imperfect CSI ([[csi-estimation-error]]). Related to the broader [[physical-layer-security]] theme.
