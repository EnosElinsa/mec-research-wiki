...
The pattern is strongest in the sources that state [[centralized-training-decentralized-execution|CTDE]] explicitly: [[zhang-2025-mcma-task-migration]] for vehicular task migration across edge servers, [[peng-2025-drudm-cfg]] for post-disaster HAS-UAV aerial MEC, and [[kang-2023-mappo-hierarchical-aerial]] for UAV-HAP hierarchical aerial computing. [[qin-2025-bcuav-masac]] uses multi-agent SAC inside a Lyapunov-decomposed blockchain UAV-MEC controller, but its parse does not label the training paradigm as CTDE; it is best read as a related MASAC instantiation, not as an explicit CTDE proof point.
