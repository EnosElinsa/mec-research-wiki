...
Eight curated sources cast MEC offloading / resource allocation as a game. They use four distinct game families, each chosen for a structural reason. This page maps the formulation to the problem shape it fits, so a future reader can pick the right one rather than defaulting to "a game".
