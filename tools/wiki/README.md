...
| `process_refs.py` | Find curation process-narration (batch/pass labels, "this/same batch", dated-run references, "paper #N" ingest-order, forward-looking "future sources should land here" placement) leaked into any page except `log.md`. A paper's own "future work" section is evergreen domain content and is NOT flagged. Exit 1 if any found. | `--json` |
