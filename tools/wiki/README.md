...
| `curation_status.py` | Reconcile `raw/sources/` vs curated pages by raw-artifact path with a normalized-title fallback for stale paths; list uncurated folders; detect duplicate MinerU ingests (identical/near) across both `full.md` and title-named parses. Exit 1 if genuinely-new papers remain. | `--dupes`, `--near-ratio`, `--json` |
