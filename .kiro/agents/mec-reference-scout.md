...
- **The default is the toolkit, not the shell.** Reusable enumeration, reconciliation, counting, reference-mining/rendering/recommending, link-integrity, and process-narration checks/transforms **MUST go through `tools/wiki/` scripts and `wikilib`** — reaching for raw PowerShell/Python for any of these is the **exception, not the norm**. So before enumerating raw folders, listing curated slugs, reconciling corpus counts, parsing references/wikilinks, mining/rendering/recommending references, or scanning for process-narration, **read `tools/wiki/README.md`** and reuse what is there. Do **not** hand-write an ad-hoc PowerShell/Python equivalent when a tool or `wikilib` helper already covers it. The scripts import the shared `wikilib` (which auto-discovers the repo root) and run from the repo root as `python tools/wiki/<script>.py`; `--json PATH` writes a machine-readable report (a relative PATH lands in `.curation-out/`). **`tools/wiki/README.md` is the single source of truth for the toolkit** — do **not** rely on a script roster, flag list, or exit-code note copied into this agent file, as it would go stale as the toolkit evolves; read the README at the start of a run for the current scripts/flags/semantics, and if this file and the README ever disagree, the README wins. What matters for you (orientation only — defer to the README for exactly what each does and how to invoke it):
  - **`wikilib.py` is the foundation your reference mining builds on** — it owns repo paths, markdown enumeration, Obsidian-faithful wikilink parsing, **and raw/sources reference parsing**. Build on these helpers so the whole toolkit parses references one consistent way; if you need a helper that isn't there yet, add it to `wikilib` rather than re-implementing it locally.
  - **Reference mining/rendering/recommending lives in the maintained CLIs, not the shell** — `mine_refs.py` (extract + parse every parse's `# REFERENCES` block into structured entries), `render_refdb.py` (render the merged, deduplicated master DB + its `.json` mirror), and `recommend_refs.py` (score/rank candidates against the current wiki state and refresh `recommendations.md`). Use and extend these instead of hand-parsing references, re-deriving the DB layout, or re-implementing the ranking inline.
  - For corpus state you also lean on `curation_status.py` (which papers are already curated, to exclude from recommendations, and which raw folders exist), `corpus_counts.py` (corpus/track tallies for the rationale), and `linkcheck.py` / `process_refs.py` (the wikilink-integrity and process-narration gates, if you touch shared pages).
- **The decision rule for the gray zone — ask before you shell.** Before running any non-trivial shell command, ask: **"Is this logic reusable or likely to recur?"**
  - **If YES (or if an existing tool is close):** extend an existing `tools/wiki/` script or `wikilib` helper with a flag, or add a new parameterized `tools/wiki/<verb_noun>.py` (imports `wikilib`, argparse CLI, **no hardcoded paths/counts**, prints a human summary, supports `--json`), update `tools/wiki/README.md`, and **commit it with the reference-mining work that motivated it**.
  ...
  - **The anti-pattern to avoid:** writing a `.curation-out/check_*_vN.py` or a multi-line inline PowerShell/Python pipeline that re-parses reference blocks, re-counts pages, or re-implements ranking logic a tool already has or should have. If you catch yourself doing this for a recurring need, stop and promote it into `mine_refs.py` / `recommend_refs.py` / `wikilib` instead.
- **Extend, don't fork.** If an existing tool or `wikilib` helper is close but insufficient, **add a flag or generalize it** rather than writing a new variant. Reference parsing belongs in `mine_refs.py` / `wikilib`, DB rendering in `render_refdb.py`, and ranking in `recommend_refs.py`: if a parse case, field, or ranking signal isn't covered yet, **extend the maintained tool** (import `wikilib`, argparse CLI, no hardcoded paths/counts, print a human summary, support `--json`), update `tools/wiki/README.md`, and commit it **with** the run that motivated it — so the next scout invocation reuses a stable parser/renderer/ranker instead of re-deriving it.
...
- **The ratchet is mandatory, not aspirational.** Each invocation MUST leave the toolkit at least as capable as it found it: any one-off you were tempted to write for a recurring need gets **promoted into a maintained tool or `wikilib` helper** (with a README update), committed alongside the run. Refine an existing tool's robustness/flags and improve the README as you go. Treat the toolkit as **append/refine-only** so the workflow converges toward a fixed, stable, trusted set over time rather than being re-derived each session; once a tool is stable, prefer reusing it unchanged. Frame **"I hand-wrote a reusable reference parse / ranking in the shell instead of promoting it"** as a process defect to avoid, not a shortcut.
...
- **Use the maintained toolkit first, and ratchet it forward.** The toolkit is the **default**, not the shell: build enumeration/reconciliation on `wikilib` and the `tools/wiki/` scripts (`mine_refs.py`, `render_refdb.py`, `recommend_refs.py`, `curation_status.py`, `corpus_counts.py`) rather than hand-rolling reference parsing, DB rendering, or ranking in PowerShell/Python. Before any non-trivial shell command, ask "is this reusable or likely to recur?" — if yes (or an existing tool is close), **extend the maintained tool or `wikilib` helper** (with a README update, committed alongside the run) rather than forking it or leaving a one-off in `.curation-out/`; only a genuinely one-shot, trivial, non-reusable command may stay inline (see "Toolkit & tooling discipline").
